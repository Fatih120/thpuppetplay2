        ...
        for (i = 0, mon = gEnemyParty; i < PARTY_SIZE; mon++, i++)
        {
            ...
            if (species != SPECIES_NONE)
            {
                if (species == SPECIES_ASHOU && GetMonData(mon, MON_DATA_LANGUAGE) != LANGUAGE_JAPANESE)
                {
                    ...
                    if (!StringCompareWithoutExtCtrlCodes(name, sJPText_AShou))
                    {
...
static bool32 IsGomasekiOrAkyuuUntradable(u16 species, bool8 isObedientBitSet)
{
...
int GetUnionRoomTradeMessageId(struct GFtgtGnameSub rfuPlayer, struct GFtgtGnameSub rfuPartner, u16 playerSpecies2, u16 partnerSpecies, u8 requestedType, u16 playerSpecies, u8 isObedientBitSet)
{
    ...
    if (IsGomasekiOrAkyuuUntradable(playerSpecies, isObedientBitSet))
    {
...
int CanRegisterMonForTradingBoard(struct GFtgtGnameSub rfuPlayer, u16 species2, u16 species, u8 isObedientBitSet)
{
    ...
    if (IsGomasekiOrAkyuuUntradable(species, isObedientBitSet))
...
static void LoadTradeMonPic(u8 whichParty, u8 state)
{
    ...
    case 0:
        ...
        if (whichParty == TRADE_PLAYER)
            ...
            HandleLoadSpecialPokePic_DontHandleGomaseki(&gMonFrontPicTable[species], gMonSpritesGfxPtr->sprites[whichParty * 2 + 1], species, personality);
