...
extern const u8 FarawayIsland_Interior_EventScript_HideAkyuuWhenGrassCut[];
...
static void CutGrassSpriteCallbackEnd(struct Sprite *sprite)
{
    ...
    if (IsAkyuuPlayingHideAndSeek() == TRUE)
        ScriptContext1_SetupScript(FarawayIsland_Interior_EventScript_HideAkyuuWhenGrassCut);
}
