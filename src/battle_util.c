            ...
            if (gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION)
            {
                ...
                if (gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION)
                {
                    ...
                    else // confusion dmg
                    {
                        ...
                        gBattleMoveDamage = CalculateBaseDamage(&gBattleMons[gBattlerAttacker], &gBattleMons[gBattlerAttacker], MOVE_STRIKE, 0, 40, 0, gBattlerAttacker, gBattlerAttacker);
...
u8 IsMonDisobedient(void)
{
    ...
    if (HasObedientBitSet(gBattlerAttacker)) // only if species is Akyuu or Gomaseki
    {
