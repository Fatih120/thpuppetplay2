...
// Note that in this file 'Daiyousei Berry Picking' is often
// shortened to DaiyouseiGame or just Game for convenience
...
enum {
    PICK_NONE,     // Daiyousei standing still
    PICK_RIGHT,    // Daiyousei reaching right
    PICK_MIDDLE,   // Daiyousei reaching up
    PICK_LEFT,     // Daiyousei reaching left
    PICK_DISABLED, // Daiyousei down after game over
};
...
enum {
    BERRYSTATE_NONE,
    BERRYSTATE_PICKED,   // Berry has been picked by a Daiyousei, replaced with blue hit sprite (still falling)
...
struct DaiyouseiGame_Berries
{
    u8 ids[NUM_BERRY_COLUMNS];
    u8 fallDist[NUM_BERRY_COLUMNS];
};

struct DaiyouseiGame_PlayerCommData
{
    u8 pickState;
...
struct DaiyouseiGame_Player
{
    u8 name[16];
    bool32 receivedGameStatePacket; // Never read
    struct DaiyouseiGame_Berries berries;
    struct DaiyouseiGame_PlayerCommData comm;
    u32 unused;
}; // size = 0x3C

// Because Daiyousei is required for this minigame,
// the only relevant information about the selected
// Pokémon is whether or not it's shiny
struct DaiyouseiGame_MonInfo
{
    bool8 isShiny;
};

struct DaiyouseiGame_ScoreResults
{
    u8 ranking;
    u32 score;
};

struct DaiyouseiGame
{
    ...
    /*0x0160*/ struct DaiyouseiGame_Gfx gfx;
    /*0x318C*/ struct DaiyouseiGame_MonInfo monInfo[MAX_RFU_PLAYERS];
    /*0x31A0*/ struct DaiyouseiGame_Player players[MAX_RFU_PLAYERS];
    /*0x32CC*/ struct DaiyouseiGame_Player player;
    /*0x3308*/ struct DaiyouseiGame_ScoreResults scoreResults[MAX_RFU_PLAYERS];
}; // size = 0x3330

EWRAM_DATA static struct DaiyouseiGame * sGame = NULL;
EWRAM_DATA static u16 * sDaiyouseiSpriteIds[MAX_RFU_PLAYERS] = {NULL};
...
EWRAM_DATA static struct DaiyouseiGame_Gfx * sGfx = NULL;

static bool32 sExitingGame;

static void ResetTasksAndSprites(void);
static void InitDaiyouseiGame(struct DaiyouseiGame *);
static void Task_StartDaiyouseiGame(u8);
...
static void CB2_DaiyouseiGame(void);
static void VBlankCB_DaiyouseiGame(void);
static void InitMonInfo(struct DaiyouseiGame_MonInfo *, struct Pokemon *);
static void CreateTask_(TaskFunc, u8);
static void CreateDaiyouseiGameTask(TaskFunc);
...
static void SendPacket_GameState(struct DaiyouseiGame_Player *,
                                 struct DaiyouseiGame_PlayerCommData *,
                                 struct DaiyouseiGame_PlayerCommData *,
                                 struct DaiyouseiGame_PlayerCommData *,
                                 struct DaiyouseiGame_PlayerCommData *,
...
static void LoadDaiyouseiGfx(void);
static void CreateDaiyouseiSprite(struct DaiyouseiGame_MonInfo *, u8, u8, u8);
static void StartDaiyouseiMissedAnim(u8);
static void StartDaiyouseiIntroAnim(u8);
static void FreeDaiyouseiSprites(u8);
static void SetAllDaiyouseiInvisibility(bool8, u8);
...
static void InitGameGfx(struct DaiyouseiGame_Gfx *);
...
static void SetDaiyouseiAnim(u8, u8);
...
static void Task_ShowDaiyouseiBerryPickingRecords(u8);
static void Task_TryRunGfxFunc(u8);
static void PrintRecordsText(u8, s32);
static void SpriteCB_Status(struct Sprite *);
static void SpriteCB_Daiyousei(struct Sprite *);
static u32 DoDaiyouseiMissedAnim(struct Sprite *);
static u32 DoDaiyouseiIntroAnim(struct Sprite *);
static s16 GetDaiyouseiXPos(u8, u8);
static void SetDaiyouseiInvisibility(bool8, u8);
...
// A table for which falling berry column corresponds to which Daiyousei head for each player
// The numbers in each array are the column number for each head, {left, middle, right}
// Dependent on the number of players
static const u8 sDaiyouseiHeadToColumnMap[MAX_RFU_PLAYERS][MAX_RFU_PLAYERS][3] =
{
    { // 1 player (never used)
        {4, 5, 6},
    },
...
static const u8 sDaiyouseiNeighborMap[MAX_RFU_PLAYERS][MAX_RFU_PLAYERS][3] =
{
    { // 1 player (never used)
        {1, 0, 1},
    },
...
static const u32 sDuplicateGfx[] = INCBIN_U32("graphics/daiyousei_berry_picking/tree_border.gbapal",
                                     "graphics/daiyousei_berry_picking/bg.gbapal",
                                     "graphics/daiyousei_berry_picking/daiyousei.gbapal",
                                     "graphics/daiyousei_berry_picking/shiny.gbapal",
                                     "graphics/daiyousei_berry_picking/status.gbapal",
                                     "graphics/daiyousei_berry_picking/berries.gbapal",
                                     "graphics/daiyousei_berry_picking/berries.4bpp.lz",
                                     "graphics/daiyousei_berry_picking/cloud.gbapal",
                                     "graphics/daiyousei_berry_picking/bg.4bpp.lz",
                                     "graphics/daiyousei_berry_picking/tree_border.4bpp.lz",
                                     "graphics/daiyousei_berry_picking/status.4bpp.lz",
                                     "graphics/daiyousei_berry_picking/cloud.4bpp.lz",
                                     "graphics/daiyousei_berry_picking/daiyousei.4bpp.lz",
                                     "graphics/daiyousei_berry_picking/bg.bin.lz",
                                     "graphics/daiyousei_berry_picking/tree_border_right.bin.lz",
                                     "graphics/daiyousei_berry_picking/tree_border_left.bin.lz");
...
void StartDaiyouseiBerryPicking(u16 partyId, void (*exitCallback)(void))
{
    sExitingGame = FALSE;
    ...
    if (gReceivedRemoteLinkPlayers && (sGame = AllocZeroed(sizeof(*sGame))))
    {
        ...
        CreateTask(Task_StartDaiyouseiGame, 1);
        SetMainCallback2(CB2_DaiyouseiGame);
        SetRandomPrize();
...
static void InitDaiyouseiGame(struct DaiyouseiGame * game)
{
    u8 i;
...
static void Task_StartDaiyouseiGame(u8 taskId)
{
    u8 i, numPlayers;
    ...
    case 4:
        ...
        for (i = 0; i < numPlayers; i++)
            CreateDaiyouseiSprite(&sGame->monInfo[sGame->posToPlayerId[i]], i, sGame->posToPlayerId[i], sGame->numPlayers);

        SetAllDaiyouseiInvisibility(FALSE, sGame->numPlayers);
        sGame->startState++;
        break;
    case 5:
        LoadBerryGfx();
    ...
    case 6:
        ...
        SetVBlankCallback(VBlankCB_DaiyouseiGame);
        sGame->startState++;
        break;
    case 7:
        UpdatePaletteFade();
    ...
    default:
        DestroyTask(taskId);
        CreateDaiyouseiGameTask(Task_NewGameIntro);
        break;
    }
}

static void Task_DaiyouseiGame_Leader(u8 taskId)
{
    RecvLinkData_Leader();
...
static void Task_DaiyouseiGame_Member(u8 taskId)
{
    RecvLinkData_Member();
...
static void ExitGame(void)
{
    ...
    case 2:
        FreeBerrySprites();
        FreeStatusBar();
        FreeDaiyouseiSprites(sGame->numPlayers);
        FreeCloudSprites();
        sExitingGame = TRUE;
...
static void Task_NewGameIntro(u8 taskId)
{
    ...
    default:
        if (sGame->isLeader)
            CreateDaiyouseiGameTask(Task_DaiyouseiGame_Leader);
        else
            CreateDaiyouseiGameTask(Task_DaiyouseiGame_Member);
...
static void HandleSound_Member(void)
{
    ...
    {
        struct DaiyouseiGame_Berries * berries = &sGame->players[sGame->multiplayerId].berries;
...
static void InitMonInfo(struct DaiyouseiGame_MonInfo * monInfo, struct Pokemon *mon)
{
...
static void CreateDaiyouseiGameTask(TaskFunc func)
{
...
static void InitFirstWaveOfBerries(void)
{
    ...
    for (i = berryStart; i < berryEnd; i++)
    {
        struct DaiyouseiGame_Berries * berries = &sGame->player.berries;
...
static bool32 TryPickBerry(u8 playerId, u8 pickState, u8 column)
{
    ...
    if (berries->fallDist[column] == EAT_FALL_DIST - 1 || berries->fallDist[column] == EAT_FALL_DIST)
    {
        ...
        if (column == sDaiyouseiHeadToColumnMap[numPlayersIdx][playerId][pick])
        {
...
static void UpdateFallingBerries(void)
{
    ...
    {
        struct DaiyouseiGame *game = sGame;
...
static void UpdateBerrySprites(void)
{
    ...
    for (i = berryStart; i < berryEnd; i++)
    {
        struct DaiyouseiGame_Player *player = &sGame->players[sGame->multiplayerId];
...
static void UpdateAllDaiyouseiAnims(void)
{
    u8 i, numPlayers;
    ...
    for (i = 0; i < numPlayers; i++)
    {
        struct DaiyouseiGame_Player *player = &sGame->players[i];
        SetDaiyouseiAnim(i, player->comm.pickState);
    }
}

static void SetAllDaiyouseiDisabled(void)
{
    u8 i, numPlayers;

    numPlayers = sGame->numPlayers;
    for (i = 0; i < numPlayers; i++)
        SetDaiyouseiAnim(i, PICK_DISABLED);
}
...
static void UpdateGame_Leader(void)
{
    UpdateBerrySprites();
    if (sGame->numGraySquares >= NUM_STATUS_SQUARES)
        SetAllDaiyouseiDisabled();
    else
        UpdateAllDaiyouseiAnims();
...
static u8 GetNewBerryId(u8 playerId, u8 column)
{
    ...
    u8 numPlayersIdx = sGame->numPlayers - 1;
    u8 leftPlayer = sDaiyouseiNeighborMap[numPlayersIdx][playerId][0];
    u8 middlePlayer = sDaiyouseiNeighborMap[numPlayersIdx][playerId][1];
    u8 rightPlayer = sDaiyouseiNeighborMap[numPlayersIdx][playerId][2];
...
// The player may extend their Daiyousei's heads while they wait for
...
static void GetScoreResults(struct DaiyouseiGame_ScoreResults *dst, u8 playerId)
{
...
void ShowDaiyouseiBerryPickingRecords(void)
{
    u8 taskId = CreateTask(Task_ShowDaiyouseiBerryPickingRecords, 0);
    Task_ShowDaiyouseiBerryPickingRecords(taskId);
}
...
static void Task_ShowDaiyouseiBerryPickingRecords(u8 taskId)
{
...
static void SendPacket_GameState(struct DaiyouseiGame_Player *player,
                                 struct DaiyouseiGame_PlayerCommData *player1,
                                 struct DaiyouseiGame_PlayerCommData *player2,
                                 struct DaiyouseiGame_PlayerCommData *player3,
                                 struct DaiyouseiGame_PlayerCommData *player4,
                                 struct DaiyouseiGame_PlayerCommData *player5,
                                 u8 numGraySquares,
                                 bool32 berriesFalling,
                                 bool32 allReadyToEnd)
{
    struct GameStatePacket packet;
    struct DaiyouseiGame_Berries *berries = &player->berries;
...
static bool32 RecvPacket_GameState(u32 playerId,
                                   struct DaiyouseiGame_Player *player,
                                   struct DaiyouseiGame_PlayerCommData *player1,
                                   struct DaiyouseiGame_PlayerCommData *player2,
                                   struct DaiyouseiGame_PlayerCommData *player3,
                                   struct DaiyouseiGame_PlayerCommData *player4,
...
// Unused duplicate of sDaiyouseiHeadToColumnMap
static const u8 sDaiyouseiHeadToColumnMap_Duplicate[MAX_RFU_PLAYERS][MAX_RFU_PLAYERS][3] =
{
    {
        {4, 5, 6},
    },
...
// Unused duplicate of sDaiyouseiNeighborMap
static const u8 sDaiyouseiNeighborMap_Duplicate[MAX_RFU_PLAYERS][MAX_RFU_PLAYERS][3] =
{
    {
        {1, 0, 1},
    },
...
static const u16 sBg_Pal[]                  = INCBIN_U16("graphics/daiyousei_berry_picking/tree_border.gbapal",
                                                         "graphics/daiyousei_berry_picking/bg.gbapal");
static const u16 sDaiyouseiNormal_Pal[]        = INCBIN_U16("graphics/daiyousei_berry_picking/daiyousei.gbapal");
static const u16 sDaiyouseiShiny_Pal[]         = INCBIN_U16("graphics/daiyousei_berry_picking/shiny.gbapal");
static const u16 sStatus_Pal[]              = INCBIN_U16("graphics/daiyousei_berry_picking/status.gbapal");
static const u16 sBerries_Pal[]             = INCBIN_U16("graphics/daiyousei_berry_picking/berries.gbapal");
static const u32 sBerries_Gfx[]             = INCBIN_U32("graphics/daiyousei_berry_picking/berries.4bpp.lz");
static const u16 sCloud_Pal[]               = INCBIN_U16("graphics/daiyousei_berry_picking/cloud.gbapal");
static const u32 sBg_Gfx[]                  = INCBIN_U32("graphics/daiyousei_berry_picking/bg.4bpp.lz");
static const u32 sTreeBorder_Gfx[]          = INCBIN_U32("graphics/daiyousei_berry_picking/tree_border.4bpp.lz");
static const u32 sStatus_Gfx[]              = INCBIN_U32("graphics/daiyousei_berry_picking/status.4bpp.lz");
static const u32 sCloud_Gfx[]               = INCBIN_U32("graphics/daiyousei_berry_picking/cloud.4bpp.lz");
static const u32 sDaiyousei_Gfx[]              = INCBIN_U32("graphics/daiyousei_berry_picking/daiyousei.4bpp.lz");
static const u32 sBg_Tilemap[]              = INCBIN_U32("graphics/daiyousei_berry_picking/bg.bin.lz");
static const u32 sTreeBorderRight_Tilemap[] = INCBIN_U32("graphics/daiyousei_berry_picking/tree_border_right.bin.lz");
static const u32 sTreeBorderLeft_Tilemap[]  = INCBIN_U32("graphics/daiyousei_berry_picking/tree_border_left.bin.lz");

static const struct OamData sOamData_Daiyousei =
{
    .y = 0,
...
static const union AnimCmd sAnim_Daiyousei_Normal[] =
{
    ANIMCMD_FRAME(0, 20),
    ANIMCMD_JUMP(0)
};

static const union AnimCmd sAnim_Daiyousei_PickRight[] =
{
    ANIMCMD_FRAME(64, 20),
    ANIMCMD_JUMP(0)
};

static const union AnimCmd sAnim_Daiyousei_PickMiddle[] =
{
    ANIMCMD_FRAME(128, 20),
    ANIMCMD_JUMP(0)
};

static const union AnimCmd sAnim_Daiyousei_PickLeft[] =
{
    ANIMCMD_FRAME(192, 20),
    ANIMCMD_JUMP(0)
};

static const union AnimCmd sAnim_Daiyousei_Down[] =
{
    ANIMCMD_FRAME(256, 20),
    ANIMCMD_JUMP(0)
};

static const union AnimCmd *const sAnims_Daiyousei[] =
{
    [PICK_NONE]     = sAnim_Daiyousei_Normal,
    [PICK_RIGHT]    = sAnim_Daiyousei_PickRight,
    [PICK_MIDDLE]   = sAnim_Daiyousei_PickMiddle,
    [PICK_LEFT]     = sAnim_Daiyousei_PickLeft,
    [PICK_DISABLED] = sAnim_Daiyousei_Down,
    // There is an unused 6th frame of Daiyousei's graphic
};
...
static void LoadDaiyouseiGfx(void)
{
    ...
    struct SpritePalette normal = {sDaiyouseiNormal_Pal, PALTAG_DAIYOUSEI_NORMAL};
    struct SpritePalette shiny = {sDaiyouseiShiny_Pal, PALTAG_DAIYOUSEI_SHINY};

    LZ77UnCompWram(sDaiyousei_Gfx, ptr);
    if (ptr)
    {
        struct SpriteSheet sheet = {ptr, 0x3000, GFXTAG_DAIYOUSEI};
...
static void CreateDaiyouseiSprite(struct DaiyouseiGame_MonInfo * monInfo, u8 playerId, u8 id, u8 numPlayers)
{
    struct SpriteTemplate template =
    {
        .tileTag = GFXTAG_DAIYOUSEI,
        .paletteTag = monInfo->isShiny, // PALTAG_DAIYOUSEI_NORMAL / PALTAG_DAIYOUSEI_SHINY
        .oam = &sOamData_Daiyousei,
        .anims = sAnims_Daiyousei,
        .images = NULL,
        .affineAnims = gDummySpriteAffineAnimTable,
        .callback = SpriteCB_Daiyousei,
    };

    sDaiyouseiSpriteIds[id] = AllocZeroed(4);
    *sDaiyouseiSpriteIds[id] = CreateSprite(&template, GetDaiyouseiXPos(playerId, numPlayers), 136, 3);
    SetDaiyouseiInvisibility(TRUE, id);
}
...
static void SpriteCB_Daiyousei(struct Sprite *sprite)
{
    ...
    case 0:
        break;
    case 1:
        DoDaiyouseiMissedAnim(sprite);
        break;
    case 2:
        DoDaiyouseiIntroAnim(sprite);
        break;
    }
}

static void StartDaiyouseiMissedAnim(u8 unused)
{
    struct Sprite *sprite = &gSprites[*sDaiyouseiSpriteIds[GetMultiplayerId()]];
    sprite->sState = 1;
    sprite->sTimer = 0;
    sprite->sUnused1 = 0;
    sprite->sUnused2 = 0;
...
static void StartDaiyouseiIntroAnim(u8 unused)
{
    struct Sprite *sprite = &gSprites[*sDaiyouseiSpriteIds[GetMultiplayerId()]];
    sprite->sState = 2;
    sprite->sTimer = 0;
...
// Do animation where Daiyousei shakes horizontally after reaching for a berry and missing
static u32 DoDaiyouseiMissedAnim(struct Sprite *sprite)
{
    s8 x;
    ...
    if (sprite->sTimer >= 3)
    {
        ...
        {
            sprite->sState = 0;
            sprite->x = GetDaiyouseiXPos(0, GetNumPlayers());
        }
    }
...
// Does the intro animation where the player's Daiyousei
...
static u32 DoDaiyouseiIntroAnim(struct Sprite *sprite)
{
    ...
    SetDaiyouseiAnim(GetMultiplayerId(), pickState);
    return 0;
}
...
static void FreeDaiyouseiSprites(u8 numPlayers)
{
    u8 i;
    for (i = 0; i < numPlayers; i++)
    {
        struct Sprite *sprite = &gSprites[*sDaiyouseiSpriteIds[i]];
        if (sprite)
            DestroySpriteAndFreeResources(sprite);
#ifdef BUGFIX
        FREE_AND_SET_NULL(sDaiyouseiSpriteIds[i]); // Memory should be freed here but is not.
#endif
    }
}

static void SetDaiyouseiInvisibility(bool8 invisible, u8 id)
{
    gSprites[*sDaiyouseiSpriteIds[id]].invisible = invisible;
}

static void SetAllDaiyouseiInvisibility(bool8 invisible, u8 count)
{
    u8 i;
    for (i = 0; i < count; i++)
        SetDaiyouseiInvisibility(invisible, i);
}

static void SetDaiyouseiAnim(u8 id, u8 pickState)
{
    StartSpriteAnim(&gSprites[*sDaiyouseiSpriteIds[id]], pickState);
}
...
static s16 GetDaiyouseiXPos(u8 playerId, u8 numPlayers)
{
    s16 x = 0;
    switch (numPlayers)
    {
...
static void InitGameGfx(struct DaiyouseiGame_Gfx *ptr)
{
    sGfx = ptr;
    sGfx->finished = FALSE;
    sGfx->state = 0;
...
static void PrintRankedScores(u8 numPlayers_)
{
    ...
    struct DaiyouseiGame_ScoreResults temp, scoreResults[MAX_RFU_PLAYERS];
