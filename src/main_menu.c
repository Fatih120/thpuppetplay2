 ...
 * Task_NewGameBirchSpeech_StartBirchAHinaPlatformFade
 * Task_NewGameBirchSpeech_StartBirchAHinaPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchAHina
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForAHina
 * Task_NewGameBirchSpeechSub_WaitForAHina
...
static void Task_NewGameBirchSpeechSub_WaitForAHina(u8);
static void Task_NewGameBirchSpeech_StartBirchAHinaPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchAHina(u8);
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tAHinaSpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_AHINA);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForAHina;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForAHina(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tAHinaSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchAHinaPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchAHinaPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tAHinaSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tAHinaSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tAHinaSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchAHina;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchAHina(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tAHinaSpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tAHinaSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tAHinaSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateAHinaSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_AHINA, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    ahinaSpriteId = NewGameBirchSpeech_CreateAHinaSprite(100, 0x4B);
    gSprites[ahinaSpriteId].callback = SpriteCB_Null;
    gSprites[ahinaSpriteId].oam.priority = 0;
    gSprites[ahinaSpriteId].invisible = TRUE;
    gTasks[taskId].tAHinaSpriteId = ahinaSpriteId;
