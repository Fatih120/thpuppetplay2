...
static void CreateBattlerSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            ...
            if (gBattleSpritesDataPtr->battlerData[battler].transformSpecies == SPECIES_TSANAE)
                gSprites[gBattlerSpriteIds[battler]].anims = gMonFrontAnimsPtrTable[SPECIES_TSANAE];
        }
