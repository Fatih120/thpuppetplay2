#include "easy_chat.h"

#define DOUBLE_SPECIES_NAME EC_EMPTY_WORD, 2,

const u16 gEasyChatWordsByLetter_Others[] = {
    EC_WORD_EXCL,
    EC_WORD_EXCL_EXCL,
    EC_WORD_DASH,
    EC_WORD_DASH_DASH_DASH,
    EC_WORD_ELLIPSIS,
    EC_WORD_ELLIPSIS_EXCL,
    EC_WORD_ELLIPSIS_ELLIPSIS_ELLIPSIS,
    EC_WORD_1_HIT_KO_EXCL,
    EC_WORD_QUES,
    EC_WORD_QUES_EXCL,
};

const u16 gEasyChatWordsByLetter_A[] = {
    EC_WORD_A,
    EC_WORD_A_LITTLE,
    EC_WORD_A_LOT,
    EC_WORD_A_TINY_BIT,
    EC_WORD_ABOUT,
    EC_WORD_ABOVE,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(LETTY),
    EC_POKEMON_NATIONAL(LETTY),
    EC_WORD_ABSENT,
    EC_POKEMON(SMEDICINE),
    EC_WORD_ABSOLUTELY,
    EC_MOVE(ABSORB),
    EC_WORD_ACCEPT,
    EC_MOVE2(ACID),
    EC_MOVE2(ACID_ARMOR),
    EC_WORD_ADORE,
    EC_WORD_ADULT,
    EC_WORD_ADVENTURE,
    EC_MOVE2(AERIAL_ACE),
    EC_MOVE(AEROBLAST),
    EC_POKEMON_NATIONAL(CBYAKUREN),
    EC_WORD_AFTER,
    EC_WORD_AGE,
    EC_POKEMON(ADALICE),
    EC_MOVE(AGILITY),
    EC_WORD_AGREE,
    EC_WORD_AHAHA,
    EC_WORD_AIM,
    EC_POKEMON_NATIONAL(CRIKA),
    EC_MOVE(AIR_CUTTER),
    EC_WORD_AIR_LOCK,
    EC_WORD_AIYEEH,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(PATCHOULI),
    EC_POKEMON_NATIONAL(PATCHOULI),
    EC_WORD_ALL,
    EC_WORD_ALL_RIGHT,
    EC_WORD_ALLOW,
    EC_WORD_ALLOWANCE,
    EC_WORD_ALLY,
    EC_WORD_ALMOST,
    EC_WORD_ALONE,
    EC_WORD_ALSO,
    EC_WORD_ALT_COLOR,
    EC_POKEMON(TIKU),
    EC_WORD_ALTHOUGH,
    EC_WORD_ALWAYS,
    EC_WORD_AM,
    EC_MOVE2(AMNESIA),
    EC_POKEMON_NATIONAL(KIRISAME),
    EC_WORD_AMUSING,
    EC_WORD_AN,
    EC_MOVE(ANCIENT_POWER),
    EC_WORD_AND,
    EC_WORD_ANGER,
    EC_WORD_ANGERS,
    EC_WORD_ANGRY,
    EC_WORD_ANIME,
    EC_POKEMON(ADCHEN),
    EC_WORD_ANOTHER,
    EC_WORD_ANTICIPATION,
    EC_WORD_ANY,
    EC_WORD_ANYWHERE,
    EC_WORD_APOLOGIZE,
    EC_WORD_APPEAL,
    EC_WORD_APPEAR,
    EC_WORD_APPEARS,
    EC_WORD_APPRECIATE,
    EC_WORD_APPROVED,
    EC_POKEMON_NATIONAL(REMILIA),
    EC_POKEMON_NATIONAL(MOKOU),
    EC_WORD_ARE,
    EC_WORD_AREN_T,
    EC_WORD_ARENA_TRAP,
    EC_POKEMON_NATIONAL(CMEIRA),
    EC_MOVE2(ARM_THRUST),
    EC_POKEMON(DARKALICE),
    EC_MOVE(AROMATHERAPY),
    EC_POKEMON(ADTEWI),
    EC_WORD_AROUND,
    EC_WORD_ARRGH,
    EC_POKEMON_NATIONAL(KONNGARA),
    EC_WORD_AS,
    EC_WORD_AS_IF,
    EC_WORD_AS_MUCH_AS,
    EC_MOVE2(ASSIST),
    EC_MOVE(ASTONISH),
    EC_WORD_AT,
    EC_WORD_ATTACK,
    EC_MOVE2(ATTRACT),
    EC_WORD_AUNT,
    EC_MOVE(AURORA_BEAM),
    EC_WORD_AVANT_GARDE,
    EC_WORD_AWAY,
    EC_WORD_AWESOME,
    EC_WORD_AWFUL,
    EC_WORD_AWFULLY,
    EC_WORD_AWW,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CYUMEKO),
    EC_POKEMON_NATIONAL(CYUMEKO),
    EC_POKEMON(CSHINGYOKU),
};

const u16 gEasyChatWordsByLetter_B[] = {
    EC_WORD_BABE,
    EC_WORD_BABY,
    EC_WORD_BACK,
    EC_WORD_BAD,
    EC_WORD_BADLY,
    EC_WORD_BAG,
    EC_POKEMON(KYOUKO),
    EC_WORD_BALL,
    EC_POKEMON(HKOAKUMA),
    EC_POKEMON(TYAMAME),
    EC_WORD_BANG,
    EC_POKEMON(THATATE),
    EC_MOVE2(BARRAGE),
    EC_MOVE2(BARRIER),
    EC_WORD_BATH,
    EC_MOVE2(BATON_PASS),
    EC_WORD_BATTLE,
    EC_WORD_BATTLE_ARMOR,
    EC_WORD_BATTLE_ROOM,
    EC_WORD_BATTLE_TOWER,
    EC_POKEMON_NATIONAL(TORI),
    EC_WORD_BE,
    EC_WORD_BEAT,
    EC_MOVE2(BEAT_UP),
    EC_POKEMON(HRAN),
    EC_WORD_BEAUTIFUL,
    EC_WORD_BEAUTY,
    EC_WORD_BECOMES,
    EC_POKEMON_NATIONAL(CMEDICINE),
    EC_WORD_BEEN,
    EC_WORD_BEFORE,
    EC_WORD_BEGINNING,
    EC_WORD_BEING,
    EC_POKEMON(CSEIGA),
    EC_WORD_BELIEVE,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CKOTOHIME),
    EC_POKEMON_NATIONAL(CKOTOHIME),
    EC_POKEMON_NATIONAL(TENSHI),
    EC_MOVE2(BELLY_DRUM),
    EC_WORD_BELONGS_TO,
    EC_WORD_BELOW,
    EC_WORD_BERRY,
    EC_WORD_BESIDE,
    EC_WORD_BEST,
    EC_WORD_BETTER,
    EC_MOVE(BIDE),
    EC_WORD_BIG,
    EC_WORD_BIKE,
    EC_MOVE(BIND),
    EC_MOVE(BITE),
    EC_MOVE2(BLAST_BURN),
    EC_POKEMON_NATIONAL(CLILYBLACK),
    EC_WORD_BLAZE,
    EC_MOVE2(BLAZE_KICK),
    EC_POKEMON(AREIMU),
    EC_WORD_BLEND,
    EC_POKEMON_NATIONAL(SHINGYOKUM),
    EC_MOVE2(BLIZZARD),
    EC_MOVE2(BLOCK),
    EC_WORD_BOARD,
    EC_MOVE2(BODY_SLAM),
    EC_MOVE2(BONE_CLUB),
    EC_MOVE2(BONE_RUSH),
    EC_MOVE2(BONEMERANG),
    EC_WORD_BOOK,
    EC_WORD_BORED,
    EC_WORD_BORING,
    EC_MOVE2(BOUNCE),
    EC_WORD_BOY,
    EC_WORD_BRAG,
    EC_WORD_BREAK,
    EC_POKEMON(SYAMAME),
    EC_MOVE(BRICK_BREAK),
    EC_WORD_BROTHER,
    EC_MOVE(BUBBLE),
    EC_MOVE2(BUBBLE_BEAM),
    EC_WORD_BUG,
    EC_POKEMON_NATIONAL(CSANAE),
    EC_MOVE2(BULK_UP),
    EC_MOVE2(BULLET_SEED),
    EC_WORD_BUSY,
    EC_WORD_BUT,
    EC_POKEMON_NATIONAL(MOMIJI),
    EC_WORD_BYE_BYE,
};

const u16 gEasyChatWordsByLetter_C[] = {
    EC_POKEMON(ANUE),
    EC_POKEMON(TNUE),
    EC_MOVE2(CALM_MIND),
    EC_WORD_CAME,
    EC_WORD_CAMERA,
    EC_POKEMON(CYUUGENMAGAN),
    EC_MOVE2(CAMOUFLAGE),
    EC_WORD_CAN,
    EC_WORD_CAN_WIN,
    EC_WORD_CAN_T,
    EC_WORD_CAN_T_WIN,
    EC_WORD_CANCEL,
    EC_WORD_CAPABLE,
    EC_WORD_CARDS,
    EC_WORD_CARE,
    EC_WORD_CARES,
    EC_POKEMON(ATEWI),
    EC_POKEMON(AMEDICINE),
    EC_WORD_CASE,
    EC_POKEMON(TSANAE),
    EC_POKEMON_NATIONAL(LILYBLACK),
    EC_WORD_CAUSE,
    EC_POKEMON_NATIONAL(AEIRIN),
    EC_WORD_CENTER,
    EC_WORD_CHALLENGE,
    EC_WORD_CHANGE,
    EC_WORD_CHANNEL,
    EC_POKEMON_NATIONAL(ICHIRIN),
    EC_MOVE(CHARGE),
    EC_POKEMON_NATIONAL(REISEN),
    EC_MOVE(CHARM),
    EC_POKEMON_NATIONAL(ALICE),
    EC_POKEMON_NATIONAL(CREISEN),
    EC_WORD_CHAT,
    EC_POKEMON_NATIONAL(GENJII),
    EC_WORD_CHILD_S_PLAY,
    EC_WORD_CHILDREN,
    EC_POKEMON(CWAKASAGI),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CSARA),
    EC_POKEMON_NATIONAL(CSARA),
    EC_WORD_CHLOROPHYLL,
    EC_WORD_CHOICE,
    EC_WORD_CHOOSE,
    EC_WORD_CHORES,
    EC_MOVE(CLAMP),
    EC_POKEMON(AMURASA),
    EC_WORD_CLASS,
    EC_POKEMON(SMEILING),
    EC_WORD_CLEAR_BODY,
    EC_POKEMON_NATIONAL(YUKARI),
    EC_POKEMON_NATIONAL(CYUKARI),
    EC_POKEMON_NATIONAL(LUIZE),
    EC_WORD_CLOSE,
    EC_WORD_CLOUD_NINE,
    EC_POKEMON_NATIONAL(KOMACHI),
    EC_WORD_COLD,
    EC_WORD_COLLECT,
    EC_WORD_COLLECTION,
    EC_WORD_COLOR,
    EC_WORD_COLOR_CHANGE,
    EC_POKEMON(TALICE),
    EC_WORD_COME,
    EC_WORD_COME_ON,
    EC_WORD_COME_OVER,
    EC_MOVE2(COMET_PUNCH),
    EC_WORD_COMICS,
    EC_WORD_COMMEMORATE,
    EC_WORD_COMPLETE,
    EC_WORD_COMPOUNDEYES,
    EC_WORD_CONCEDE,
    EC_MOVE(CONFUSE_RAY),
    EC_WORD_CONFUSED,
    EC_MOVE2(CONFUSION),
    EC_WORD_CONGRATS,
    EC_MOVE(CONSTRICT),
    EC_WORD_CONTEST,
    EC_WORD_CONVERSATION,
    EC_MOVE2(CONVERSION),
    EC_MOVE2(CONVERSION_2),
    EC_WORD_COOL,
    EC_WORD_COOLNESS,
    EC_POKEMON(DLAYLA),
    EC_WORD_CORRECT,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(SYOUMU),
    EC_POKEMON_NATIONAL(SYOUMU),
    EC_MOVE(COSMIC_POWER),
    EC_MOVE2(COTTON_SPORE),
    EC_WORD_COULD,
    EC_WORD_COULDN_T,
    EC_WORD_COUNT_ON,
    EC_MOVE(COUNTER),
    EC_WORD_COUPLE,
    EC_MOVE2(COVET),
    EC_MOVE(CRABHAMMER),
    EC_POKEMON(ADRAN),
    EC_POKEMON(DKEINE),
    EC_WORD_CRIES,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(MEIRA),
    EC_POKEMON_NATIONAL(MEIRA),
    EC_POKEMON_NATIONAL(YOUKI),
    EC_MOVE(CROSS_CHOP),
    EC_MOVE(CRUNCH),
    EC_WORD_CRUSH,
    EC_MOVE2(CRUSH_CLAW),
    EC_WORD_CRY,
    EC_POKEMON_NATIONAL(CLAYLA),
    EC_WORD_CURRENT,
    EC_MOVE2(CURSE),
    EC_MOVE(CUT),
    EC_WORD_CUTE,
    EC_WORD_CUTE_CHARM,
    EC_WORD_CUTENESS,
    EC_POKEMON_NATIONAL(CELLY),
};

const u16 gEasyChatWordsByLetter_D[] = {
    EC_WORD_DAILY,
    EC_WORD_DAMP,
    EC_WORD_DANCE,
    EC_WORD_DANGER,
    EC_WORD_DARK,
    EC_WORD_DARN,
    EC_WORD_DATE,
    EC_WORD_DAUGHTER,
    EC_WORD_DAY,
    EC_WORD_DAYS,
    EC_WORD_DAYTIME,
    EC_WORD_DECIDE,
    EC_WORD_DECIDED,
    EC_WORD_DEEP,
    EC_WORD_DEFEATED,
    EC_MOVE2(DEFENSE_CURL),
    EC_POKEMON(ALUNACHILD),
    EC_POKEMON_NATIONAL(SAYA),
    EC_POKEMON(GOMASEKI),
    EC_WORD_DEPT_STORE,
    EC_WORD_DESIGN,
    EC_WORD_DESTINY,
    EC_MOVE2(DESTINY_BOND),
    EC_WORD_DESTROYED,
    EC_MOVE2(DETECT),
    EC_POKEMON_NATIONAL(NITORI),
    EC_WORD_DID,
    EC_WORD_DIDN_T,
    EC_WORD_DIET,
    EC_WORD_DIFFERENT,
    EC_MOVE(DIG),
    EC_WORD_DIGITAL,
    EC_POKEMON_NATIONAL(CYAMAME),
    EC_WORD_DIRECT,
    EC_MOVE(DISABLE),
    EC_WORD_DISAPPEAR,
    EC_WORD_DISAPPOINT,
    EC_WORD_DISAPPOINTED,
    EC_WORD_DISAPPOINTS,
    EC_WORD_DISASTER,
    EC_WORD_DISLIKE,
    EC_WORD_DISLIKES,
    EC_POKEMON_NATIONAL(KEDAMA),
    EC_MOVE2(DIVE),
    EC_MOVE2(DIZZY_PUNCH),
    EC_WORD_DO,
    EC_WORD_DOCILE,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(DAIYOUSEI),
    EC_POKEMON_NATIONAL(DAIYOUSEI),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CDAIYOUSEI),
    EC_POKEMON_NATIONAL(CDAIYOUSEI),
    EC_WORD_DOES,
    EC_WORD_DOESN_T,
    EC_WORD_DON_T,
    EC_WORD_DONE,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(SKOISHI),
    EC_POKEMON_NATIONAL(SKOISHI),
    EC_MOVE2(DOOM_DESIRE),
    EC_MOVE2(DOUBLE_KICK),
    EC_MOVE(DOUBLE_TEAM),
    EC_MOVE(DOUBLE_EDGE),
    EC_MOVE(DOUBLE_SLAP),
    EC_WORD_DOWN,
    EC_WORD_DOWNCAST,
    EC_WORD_DRAGON,
    EC_MOVE2(DRAGON_CLAW),
    EC_MOVE2(DRAGON_DANCE),
    EC_MOVE2(DRAGON_RAGE),
    EC_POKEMON_NATIONAL(TOYOHIME),
    EC_MOVE2(DRAGON_BREATH),
    EC_POKEMON_NATIONAL(GOLIATH),
    EC_POKEMON_NATIONAL(CTOYOHIME),
    EC_WORD_DREAM,
    EC_MOVE2(DREAM_EATER),
    EC_MOVE2(DRILL_PECK),
    EC_WORD_DRINK,
    EC_WORD_DRINKS,
    EC_WORD_DRIZZLE,
    EC_WORD_DROOLING,
    EC_WORD_DROUGHT,
    EC_POKEMON_NATIONAL(CSAKUYA),
    EC_WORD_DUDE,
    EC_POKEMON_NATIONAL(YAMAME),
    EC_POKEMON_NATIONAL(HSTARSAPPHIRE),
    EC_POKEMON(ADCIRNO),
    EC_POKEMON(RINNOSUKE),
    EC_POKEMON(TMEDICINE),
    EC_MOVE2(DYNAMIC_PUNCH),
};

const u16 gEasyChatWordsByLetter_E[] = {
    EC_WORD_EARLIER,
    EC_WORD_EARLY_BIRD,
    EC_MOVE(EARTHQUAKE),
    EC_WORD_EASY,
    EC_WORD_EAT,
    EC_WORD_EATS,
    EC_WORD_EEK,
    EC_POKEMON_NATIONAL(CNUE),
    EC_WORD_EFFECT_SPORE,
    EC_WORD_EGG,
    EC_MOVE2(EGG_BOMB),
    EC_WORD_EH_QUES,
    EC_WORD_EHEHE,
    EC_POKEMON_NATIONAL(CREMILIA),
    EC_POKEMON_NATIONAL(SHIKIEIKI),
    EC_WORD_ELECTRIC,
    EC_POKEMON(ASHIKIEIKI),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(MERLIN),
    EC_POKEMON_NATIONAL(MERLIN),
    EC_POKEMON_NATIONAL(AKAGUYA),
    EC_WORD_ELSE,
    EC_MOVE2(EMBER),
    EC_WORD_EMERALD,
    EC_MOVE(ENCORE),
    EC_WORD_ENCYCLOPEDIA,
    EC_WORD_END,
    EC_MOVE(ENDEAVOR),
    EC_MOVE(ENDURE),
    EC_WORD_ENJOY,
    EC_WORD_ENJOYS,
    EC_WORD_ENOUGH,
    EC_POKEMON_NATIONAL(AUTSUHO),
    EC_WORD_ENTERTAINING,
    EC_MOVE2(ERUPTION),
    EC_WORD_ESCAPE,
    EC_POKEMON_NATIONAL(CKURUMI),
    EC_WORD_EVEN_SO,
    EC_WORD_EVENT,
    EC_WORD_EVENTS,
    EC_WORD_EVER,
    EC_WORD_EVERY,
    EC_WORD_EVOLUTION,
    EC_WORD_EVOLVE,
    EC_WORD_EXCELLENT,
    EC_WORD_EXCESS,
    EC_WORD_EXCITE,
    EC_WORD_EXCITING,
    EC_WORD_EXCUSE,
    EC_WORD_EXCUSE_ME,
    EC_POKEMON_NATIONAL(CLYRICA),
    EC_POKEMON_NATIONAL(LYRICA),
    EC_WORD_EXISTS,
    EC_WORD_EXPENSIVE,
    EC_MOVE2(EXPLOSION),
    EC_POKEMON(DMURASA),
    EC_MOVE(EXTRASENSORY),
    EC_MOVE(EXTREME_SPEED),
};

const u16 gEasyChatWordsByLetter_F[] = {
    EC_WORD_FABULOUS,
    EC_MOVE(FACADE),
    EC_WORD_FAINT,
    EC_MOVE2(FAINT_ATTACK),
    EC_WORD_FAINTED,
    EC_MOVE2(FAKE_OUT),
    EC_MOVE(FAKE_TEARS),
    EC_WORD_FALL,
    EC_MOVE2(FALSE_SWIPE),
    EC_WORD_FAMILY,
    EC_WORD_FANTASTIC,
    EC_WORD_FAR,
    EC_POKEMON_NATIONAL(RAN),
    EC_WORD_FASHION,
    EC_WORD_FAST,
    EC_WORD_FATHER,
    EC_WORD_FEAR,
    EC_POKEMON_NATIONAL(TOKIKO),
    EC_MOVE2(FEATHER_DANCE),
    EC_POKEMON(AREMILIA),
    EC_WORD_FEELING,
    EC_POKEMON_NATIONAL(CCHIYURI),
    EC_WORD_FESTIVAL,
    EC_WORD_FEVER,
    EC_WORD_FIERY,
    EC_WORD_FIGHT,
    EC_WORD_FIGHTING,
    EC_WORD_FIGHTS,
    EC_WORD_FINAL,
    EC_WORD_FINALLY,
    EC_WORD_FINISH,
    EC_WORD_FIRE,
    EC_MOVE2(FIRE_BLAST),
    EC_MOVE2(FIRE_PUNCH),
    EC_MOVE2(FIRE_SPIN),
    EC_WORD_FISHING,
    EC_MOVE(FISSURE),
    EC_POKEMON_NATIONAL(CKIRISAME),
    EC_MOVE(FLAIL),
    EC_WORD_FLAME,
    EC_WORD_FLAME_BODY,
    EC_MOVE(FLAME_WHEEL),
    EC_MOVE(FLAMETHROWER),
    EC_POKEMON_NATIONAL(SUWAKO),
    EC_MOVE2(FLASH),
    EC_WORD_FLASH_FIRE,
    EC_WORD_FLATTEN,
    EC_MOVE(FLATTER),
    EC_WORD_FLOWERS,
    EC_MOVE(FLY),
    EC_POKEMON(ASANAE),
    EC_WORD_FLYING,
    EC_MOVE(FOCUS_ENERGY),
    EC_MOVE(FOCUS_PUNCH),
    EC_WORD_FOE,
    EC_MOVE(FOLLOW_ME),
    EC_WORD_FOR,
    EC_WORD_FOR_NOW,
    EC_WORD_FORECAST,
    EC_MOVE2(FORESIGHT),
    EC_WORD_FOREVER,
    EC_WORD_FORGET,
    EC_WORD_FORGETS,
    EC_WORD_FORGIVE,
    EC_POKEMON_NATIONAL(TLUNACHILD),
    EC_MOVE2(FRENZY_PLANT),
    EC_WORD_FRIDAY,
    EC_WORD_FRIEND,
    EC_WORD_FROM,
    EC_WORD_FRONT,
    EC_WORD_FRONTIER,
    EC_MOVE2(FRUSTRATION),
    EC_WORD_FUFUFU,
    EC_WORD_FULL,
    EC_WORD_FUNNY,
    EC_POKEMON_NATIONAL(CYUMEMI),
    EC_MOVE2(FURY_ATTACK),
    EC_MOVE2(FURY_CUTTER),
    EC_MOVE2(FURY_SWIPES),
    EC_MOVE2(FUTURE_SIGHT),
};

const u16 gEasyChatWordsByLetter_G[] = {
    EC_WORD_GAME,
    EC_POKEMON(CKYOUKO),
    EC_POKEMON_NATIONAL(CYUYUKO),
    EC_POKEMON_NATIONAL(CMEILING),
    EC_WORD_GENIUS,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CKISUME),
    EC_POKEMON_NATIONAL(CKISUME),
    EC_WORD_GET,
    EC_WORD_GETS,
    EC_WORD_GHOST,
    EC_WORD_GIDDY,
    EC_MOVE(GIGA_DRAIN),
    EC_WORD_GIGGLE,
    EC_WORD_GIMME,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(MAI),
    EC_POKEMON_NATIONAL(MAI),
    EC_WORD_GIRL,
    EC_WORD_GIVE,
    EC_WORD_GIVE_ME,
    EC_WORD_GIVE_UP,
    EC_WORD_GIVES,
    EC_POKEMON(DLETTY),
    EC_MOVE2(GLARE),
    EC_POKEMON_NATIONAL(ANITORI),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(KEINE),
    EC_POKEMON_NATIONAL(KEINE),
    EC_WORD_GO,
    EC_WORD_GO_AHEAD,
    EC_WORD_GO_EASY,
    EC_WORD_GO_HOME,
    EC_WORD_GOES,
    EC_WORD_GOING,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(MINORIKO),
    EC_POKEMON_NATIONAL(MINORIKO),
    EC_WORD_GOLD,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CSTARSAPPHIRE),
    EC_POKEMON_NATIONAL(CSTARSAPPHIRE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(YORIHIME),
    EC_POKEMON_NATIONAL(YORIHIME),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CKOGASA),
    EC_POKEMON_NATIONAL(CKOGASA),
    EC_WORD_GOOD,
    EC_WORD_GOOD_BYE,
    EC_POKEMON(AKAZAMI),
    EC_WORD_GOT,
    EC_WORD_GOTCHA,
    EC_WORD_GOURMET,
    EC_WORD_GRAAAH,
    EC_POKEMON_NATIONAL(DMOKOU),
    EC_WORD_GRANDFATHER,
    EC_WORD_GRANDMOTHER,
    EC_WORD_GRASS,
    EC_MOVE(GRASS_WHISTLE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(KISUME),
    EC_POKEMON_NATIONAL(KISUME),
    EC_WORD_GREAT,
    EC_WORD_GREEN,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CHINA),
    EC_POKEMON_NATIONAL(CHINA),
    EC_POKEMON(MIKO),
    EC_WORD_GROUND,
    EC_WORD_GROUP,
    EC_POKEMON(DYUKARI),
    EC_MOVE2(GROWL),
    EC_POKEMON_NATIONAL(CMOKOU),
    EC_MOVE(GROWTH),
    EC_MOVE(GRUDGE),
    EC_POKEMON(DYUUGI),
    EC_WORD_GUARD,
    EC_MOVE2(GUILLOTINE),
    EC_POKEMON(CKIKURI),
    EC_MOVE(GUST),
    EC_WORD_GUTS,
    EC_WORD_GUTSY,
    EC_WORD_GWAH,
    EC_WORD_GWAHAHAHA,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CTEWI),
    EC_POKEMON_NATIONAL(CTEWI),
};

const u16 gEasyChatWordsByLetter_H[] = {
    EC_WORD_HABIT,
    EC_WORD_HAH,
    EC_WORD_HAHAHA,
    EC_MOVE(HAIL),
    EC_WORD_HALL_OF_FAME,
    EC_WORD_HAPPENING,
    EC_WORD_HAPPILY,
    EC_WORD_HAPPINESS,
    EC_WORD_HAPPY,
    EC_WORD_HARD,
    EC_MOVE(HARDEN),
    EC_POKEMON(ADAIYOUSEI),
    EC_WORD_HAS,
    EC_WORD_HASSLE,
    EC_POKEMON_NATIONAL(YUYUKO),
    EC_WORD_HAVE,
    EC_WORD_HAVEN_T,
    EC_MOVE(HAZE),
    EC_WORD_HE,
    EC_WORD_HE_S,
    EC_MOVE(HEADBUTT),
    EC_MOVE(HEAL_BELL),
    EC_WORD_HEALTHY,
    EC_WORD_HEAR,
    EC_WORD_HEARING,
    EC_WORD_HEARS,
    EC_WORD_HEART,
    EC_MOVE2(HEAT_WAVE),
    EC_WORD_HEH,
    EC_WORD_HEHE,
    EC_WORD_HEHEHE,
    EC_WORD_HELLO,
    EC_MOVE2(HELPING_HAND),
    EC_WORD_HER,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(HSAKUYA),
    EC_POKEMON_NATIONAL(HSAKUYA),
    EC_WORD_HERE,
    EC_WORD_HERE_GOES,
    EC_WORD_HERE_I_COME,
    EC_WORD_HERE_IT_IS,
    EC_WORD_HERO,
    EC_WORD_HEROINE,
    EC_WORD_HERS,
    EC_WORD_HEY,
    EC_WORD_HEY_THERE,
    EC_WORD_HEY_QUES,
    EC_WORD_HI,
    EC_MOVE2(HI_JUMP_KICK),
    EC_WORD_HIDDEN,
    EC_MOVE2(HIDDEN_POWER),
    EC_WORD_HIGH,
    EC_WORD_HIGHS,
    EC_WORD_HIM,
    EC_WORD_HIP_AND,
    EC_WORD_HIS,
    EC_WORD_HIT,
    EC_POKEMON_NATIONAL(KANAKO),
    EC_POKEMON_NATIONAL(CKANAKO),
    EC_POKEMON_NATIONAL(AKANAKO),
    EC_WORD_HIYAH,
    EC_WORD_HMM,
    EC_POKEMON_NATIONAL(AKOMACHI),
    EC_WORD_HOBBY,
    EC_WORD_HOHOHO,
    EC_WORD_HOLIDAY,
    EC_WORD_HOME,
    EC_WORD_HOO_HAH,
    EC_POKEMON_NATIONAL(YUMEMI),
    EC_WORD_HOPELESS,
    EC_POKEMON_NATIONAL(SHINKI),
    EC_MOVE2(HORN_ATTACK),
    EC_MOVE2(HORN_DRILL),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CLUNACHILD),
    EC_POKEMON_NATIONAL(CLUNACHILD),
    EC_WORD_HOT,
    EC_POKEMON_NATIONAL(DPARSEE),
    EC_POKEMON_NATIONAL(DTENSHI),
    EC_WORD_HOW,
    EC_WORD_HOW_DO,
    EC_WORD_HOWEVER,
    EC_MOVE2(HOWL),
    EC_WORD_HUGE_POWER,
    EC_WORD_HUH_QUES,
    EC_WORD_HUMPH,
    EC_WORD_HUNGRY,
    EC_POKEMON(ASTARSAPPHIRE),
    EC_WORD_HURRIED,
    EC_WORD_HUSTLE,
    EC_MOVE2(HYDRO_CANNON),
    EC_MOVE2(HYDRO_PUMP),
    EC_WORD_HYPER,
    EC_MOVE2(HYPER_BEAM),
    EC_WORD_HYPER_CUTTER,
    EC_MOVE2(HYPER_FANG),
    EC_MOVE2(HYPER_VOICE),
    EC_POKEMON_NATIONAL(SAKUYA),
    EC_MOVE(HYPNOSIS),
};

const u16 gEasyChatWordsByLetter_I[] = {
    EC_WORD_I,
    EC_WORD_I_AM,
    EC_WORD_I_CHOOSE_YOU,
    EC_WORD_I_WAS,
    EC_WORD_I_VE,
    EC_WORD_I_VE_ARRIVED,
    EC_WORD_ICE,
    EC_MOVE(ICE_BALL),
    EC_MOVE2(ICE_BEAM),
    EC_MOVE2(ICE_PUNCH),
    EC_MOVE2(ICICLE_SPEAR),
    EC_MOVE(ICY_WIND),
    EC_WORD_IDOL,
    EC_WORD_IF,
    EC_WORD_IF_I_LOSE,
    EC_WORD_IF_I_WIN,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CMARGATROID),
    EC_POKEMON_NATIONAL(CMARGATROID),
    EC_WORD_IGNORANT,
    EC_WORD_ILLUMINATE,
    EC_POKEMON(ADMOKOU),
    EC_WORD_IMMUNITY,
    EC_WORD_IMPORTANT,
    EC_WORD_IMPOSSIBLE,
    EC_MOVE2(IMPRISON),
    EC_WORD_IN,
    EC_WORD_INCREASING,
    EC_WORD_INCREDIBLE,
    EC_WORD_INFORMATION,
    EC_MOVE2(INGRAIN),
    EC_WORD_INNER_FOCUS,
    EC_WORD_INSIDE,
    EC_WORD_INSOMNIA,
    EC_WORD_INSTEAD,
    EC_WORD_INTIMIDATE,
    EC_WORD_INVINCIBLE,
    EC_MOVE2(IRON_DEFENSE),
    EC_MOVE(IRON_TAIL),
    EC_WORD_IS,
    EC_WORD_IS_IT_QUES,
    EC_WORD_ISN_T,
    EC_WORD_ISN_T_IT_QUES,
    EC_WORD_IT,
    EC_WORD_IT_S,
    EC_WORD_ITEM,
    EC_POKEMON_NATIONAL(SANAE),
};

const u16 gEasyChatWordsByLetter_J[] = {
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CMYSTIA),
    EC_POKEMON_NATIONAL(CMYSTIA),
    EC_POKEMON(KASEN),
    EC_WORD_JOKING,
    EC_POKEMON_NATIONAL(CSUWAKO),
    EC_WORD_JOY,
    EC_WORD_JUDGE,
    EC_MOVE2(JUMP_KICK),
    EC_POKEMON_NATIONAL(RUUKOTO),
    EC_WORD_JUST,
    EC_POKEMON_NATIONAL(CSHIKIEIKI),
};

const u16 gEasyChatWordsByLetter_K[] = {
    EC_POKEMON_NATIONAL(CREIMU),
    EC_POKEMON_NATIONAL(REIMU),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CPATCHOULI),
    EC_POKEMON_NATIONAL(CPATCHOULI),
    EC_POKEMON_NATIONAL(SHIZUHA),
    EC_POKEMON_NATIONAL(SUNNYMILK),
    EC_MOVE(KARATE_CHOP),
    EC_POKEMON(SWRIGGLE),
    EC_WORD_KEEN_EYE,
    EC_WORD_KID,
    EC_WORD_KIDDING,
    EC_WORD_KIND,
    EC_WORD_KINDERGARTEN,
    EC_MOVE(KINESIS),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(TPARSEE),
    EC_POKEMON_NATIONAL(TPARSEE),
    EC_POKEMON_NATIONAL(LUNASA),
    EC_POKEMON(SENDAI),
    EC_MOVE2(KNOCK_OFF),
    EC_WORD_KNOCKOUT,
    EC_WORD_KNOW,
    EC_WORD_KNOWS,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(RIN),
    EC_POKEMON_NATIONAL(RIN),
    EC_POKEMON_NATIONAL(CLUNASA),
    EC_WORD_KTHX_BYE,
    EC_POKEMON(CMIKO),
};

const u16 gEasyChatWordsByLetter_L[] = {
    EC_WORD_LACKING,
    EC_WORD_LACKS,
    EC_WORD_LADY,
    EC_POKEMON(ADMARISA),
    EC_WORD_LALALA,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(SARA),
    EC_POKEMON_NATIONAL(SARA),
    EC_POKEMON_NATIONAL(TEWI),
    EC_POKEMON_NATIONAL(DMOMIJI),
    EC_WORD_LAST,
    EC_WORD_LATE,
    EC_WORD_LATER,
    EC_POKEMON(MAMIZOU),
    EC_POKEMON(CKASEN),
    EC_WORD_LAVISH,
    EC_WORD_LAY,
    EC_WORD_LEADER,
    EC_WORD_LEADERS,
    EC_WORD_LEAF,
    EC_MOVE2(LEAF_BLADE),
    EC_WORD_LEARN,
    EC_POKEMON_NATIONAL(CKAZAMI),
    EC_POKEMON_NATIONAL(ELLEN),
    EC_MOVE(LEECH_LIFE),
    EC_MOVE2(LEECH_SEED),
    EC_MOVE2(LEER),
    EC_WORD_LEFT,
    EC_WORD_LEGEND,
    EC_WORD_LESS,
    EC_WORD_LESSONS,
    EC_WORD_LET_ME_WIN,
    EC_WORD_LET_S,
    EC_WORD_LETTER,
    EC_WORD_LEVEL,
    EC_WORD_LEVITATE,
    EC_MOVE(LICK),
    EC_POKEMON_NATIONAL(CRIN),
    EC_WORD_LIE,
    EC_WORD_LIFE,
    EC_MOVE2(LIGHT_SCREEN),
    EC_WORD_LIGHTNINGROD,
    EC_WORD_LIKE,
    EC_WORD_LIKELY_TO,
    EC_WORD_LIKES,
    EC_POKEMON(ADYUKARI),
    EC_WORD_LIMBER,
    EC_WORD_LINK,
    EC_POKEMON(HMERLIN),
    EC_WORD_LIQUID_OOZE,
    EC_WORD_LISTEN,
    EC_WORD_LISTENING,
    EC_WORD_LISTENS,
    EC_WORD_LIVING,
    EC_MOVE2(LOCK_ON),
    EC_WORD_LOCOMOTIVE,
    EC_WORD_LOL,
    EC_WORD_LOLLING,
    EC_POKEMON(DHINA),
    EC_WORD_LONESOME,
    EC_WORD_LOOK,
    EC_WORD_LOOKS,
    EC_WORD_LOSE,
    EC_WORD_LOSER,
    EC_WORD_LOSING,
    EC_WORD_LOSS,
    EC_WORD_LOST,
    EC_POKEMON(AHINA),
    EC_POKEMON(AMARISA),
    EC_WORD_LOUSY,
    EC_MOVE(LOVELY_KISS),
    EC_WORD_LOVEY_DOVEY,
    EC_WORD_LOW,
    EC_MOVE(LOW_KICK),
    EC_WORD_LOWS,
    EC_POKEMON(SSHIZUHA),
    EC_POKEMON_NATIONAL(SKOMACHI),
    EC_WORD_LUKEWARM,
    EC_POKEMON(HLETTY),
    EC_MOVE2(LUSTER_PURGE),
    EC_POKEMON(AKISISTERS),
};

const u16 gEasyChatWordsByLetter_M[] = {
    EC_MOVE2(MACH_PUNCH),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CTENSHI),
    EC_POKEMON_NATIONAL(CTENSHI),
    EC_WORD_MACHINE,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(YUUGI),
    EC_POKEMON_NATIONAL(YUUGI),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CYUUGI),
    EC_POKEMON_NATIONAL(CYUUGI),
    EC_WORD_MAGAZINE,
    EC_POKEMON_NATIONAL(DKAGUYA),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(ACHEN),
    EC_POKEMON_NATIONAL(ACHEN),
    EC_MOVE2(MAGIC_COAT),
    EC_MOVE2(MAGICAL_LEAF),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(WRIGGLE),
    EC_POKEMON_NATIONAL(WRIGGLE),
    EC_WORD_MAGMA_ARMOR,
    EC_POKEMON_NATIONAL(CSHOU),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(EIRIN),
    EC_POKEMON_NATIONAL(EIRIN),
    EC_WORD_MAGNET_PULL,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CRAN),
    EC_POKEMON_NATIONAL(CRAN),
    EC_MOVE2(MAGNITUDE),
    EC_WORD_MAIL,
    EC_WORD_MAKE,
    EC_WORD_MAKING,
    EC_POKEMON(AFLANDRE),
    EC_WORD_MAN,
    EC_POKEMON(TTOYOHIME),
    EC_POKEMON_NATIONAL(CKAGUYA),
    EC_POKEMON_NATIONAL(TAYA),
    EC_POKEMON_NATIONAL(HAKUREI),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(KOTOHIME),
    EC_POKEMON_NATIONAL(KOTOHIME),
    EC_POKEMON_NATIONAL(LAYLA),
    EC_POKEMON(SCIRNO),
    EC_WORD_MARVEL_SCALE,
    EC_POKEMON(DWRIGGLE),
    EC_WORD_MASTER,
    EC_WORD_MATCH,
    EC_WORD_MATCH_UP,
    EC_POKEMON(TKISUME),
    EC_WORD_MAYBE,
    EC_WORD_MC,
    EC_WORD_ME,
    EC_WORD_MEAN,
    EC_MOVE(MEAN_LOOK),
    EC_POKEMON(HKEINE),
    EC_MOVE2(MEDITATE),
    EC_POKEMON(AYUUGI),
    EC_WORD_MEET,
    EC_WORD_MEET_YOU,
    EC_WORD_MEETS,
    EC_WORD_MEGA,
    EC_MOVE2(MEGA_DRAIN),
    EC_MOVE2(MEGA_KICK),
    EC_MOVE2(MEGA_PUNCH),
    EC_MOVE2(MEGAHORN),
    EC_POKEMON_NATIONAL(NAMAZU),
    EC_MOVE(MEMENTO),
    EC_POKEMON_NATIONAL(CCHEN),
    EC_WORD_MESSAGE,
    EC_POKEMON(CTOJIKO),
    EC_MOVE2(METAL_CLAW),
    EC_MOVE(METAL_SOUND),
    EC_POKEMON(SEIGA),
    EC_POKEMON_NATIONAL(CMOMIJI),
    EC_MOVE(METEOR_MASH),
    EC_MOVE2(METRONOME),
    EC_POKEMON_NATIONAL(AKYUU),
    EC_POKEMON_NATIONAL(SARIEL),
    EC_POKEMON(AYUYUKO),
    EC_MOVE2(MILK_DRINK),
    EC_POKEMON(DTEWI),
    EC_POKEMON_NATIONAL(SHINGYOKUO),
    EC_MOVE2(MIMIC),
    EC_MOVE(MIND_READER),
    EC_MOVE2(MINIMIZE),
    EC_POKEMON(HNAZRIN),
    EC_WORD_MINUS,
    EC_MOVE2(MIRROR_COAT),
    EC_MOVE(MIRROR_MOVE),
    EC_POKEMON_NATIONAL(CYUKI),
    EC_WORD_MISHEARD,
    EC_WORD_MISS,
    EC_MOVE(MIST),
    EC_MOVE2(MIST_BALL),
    EC_WORD_MISTAKE,
    EC_WORD_MMM,
    EC_WORD_MODE,
    EC_WORD_MODERN,
    EC_POKEMON_NATIONAL(YUUGENMAGAN),
    EC_WORD_MOMENTUM,
    EC_WORD_MONDAY,
    EC_WORD_MONEY,
    EC_WORD_MONTH,
    EC_WORD_MOOD,
    EC_MOVE2(MOONLIGHT),
    EC_WORD_MORE,
    EC_WORD_MORNING,
    EC_MOVE(MORNING_SUN),
    EC_WORD_MOTHER,
    EC_WORD_MOVE,
    EC_WORD_MOVIE,
    EC_WORD_MR,
    EC_POKEMON_NATIONAL(CAYA),
    EC_WORD_MRS,
    EC_WORD_MUCH,
    EC_WORD_MUCH_OBLIGED,
    EC_MOVE2(MUD_SHOT),
    EC_MOVE2(MUD_SPORT),
    EC_MOVE2(MUD_SLAP),
    EC_MOVE2(MUDDY_WATER),
    EC_POKEMON(DREIMU),
    EC_WORD_MUFUFU,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(HINA),
    EC_POKEMON_NATIONAL(HINA),
    EC_POKEMON_NATIONAL(CRIKAKO),
    EC_WORD_MUSIC,
    EC_WORD_MUST_BE,
    EC_WORD_MY,
    EC_WORD_MYSELF,
    EC_WORD_MYSTERY,
};

const u16 gEasyChatWordsByLetter_N[] = {
    EC_WORD_NAME,
    EC_WORD_NAP,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(KANA),
    EC_POKEMON_NATIONAL(KANA),
    EC_WORD_NATURAL,
    EC_WORD_NATURAL_CURE,
    EC_WORD_NATURALLY,
    EC_WORD_NATURE,
    EC_MOVE(NATURE_POWER),
    EC_WORD_NEAR,
    EC_WORD_NEARLY,
    EC_WORD_NEED,
    EC_MOVE2(NEEDLE_ARM),
    EC_WORD_NEGATIVE,
    EC_WORD_NEVER,
    EC_WORD_NEW,
    EC_WORD_NEWS,
    EC_WORD_NEXT,
    EC_WORD_NICE,
    EC_POKEMON_NATIONAL(YOUMU),
    EC_POKEMON_NATIONAL(FLANDRE),
    EC_POKEMON_NATIONAL(SHANGHAI),
    EC_POKEMON_NATIONAL(HOURAI),
    EC_POKEMON_NATIONAL(CFLANDRE),
    EC_POKEMON_NATIONAL(CYOUMU),
    EC_WORD_NIGHT,
    EC_MOVE2(NIGHT_SHADE),
    EC_MOVE(NIGHTMARE),
    EC_WORD_NIGHTTIME,
    EC_POKEMON(HSHIZUHA),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(SUIKA),
    EC_POKEMON_NATIONAL(SUIKA),
    EC_POKEMON(ASUWAKO),
    EC_WORD_NITWIT,
    EC_WORD_NO,
    EC_WORD_NO_MATCH,
    EC_WORD_NO_1,
    EC_POKEMON_NATIONAL(CELLEN),
    EC_WORD_NOISY,
    EC_WORD_NON_STOP,
    EC_WORD_NONE,
    EC_WORD_NOPE,
    EC_WORD_NORMAL,
    EC_POKEMON(CELIS),
    EC_WORD_NOT,
    EC_WORD_NOT_VERY,
    EC_WORD_NOTHING,
    EC_WORD_NOW,
    EC_POKEMON(CKONNGARA),
    EC_POKEMON(ARIN),
};

const u16 gEasyChatWordsByLetter_O[] = {
    EC_WORD_OBLIVIOUS,
    EC_MOVE(OCTAZOOKA),
    EC_POKEMON_NATIONAL(TYUUKA),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CKEINE),
    EC_POKEMON_NATIONAL(CKEINE),
    EC_MOVE(ODOR_SLEUTH),
    EC_WORD_OF,
    EC_WORD_OFF,
    EC_WORD_OFFENSIVE,
    EC_WORD_OH,
    EC_WORD_OH_DEAR,
    EC_WORD_OH_YEAH,
    EC_WORD_OH_KAY,
    EC_WORD_OH_QUES,
    EC_WORD_OI,
    EC_WORD_OI_OI_OI,
    EC_WORD_OK_QUES,
    EC_WORD_OKAY,
    EC_WORD_OLD,
    EC_WORD_OLDEN,
    EC_POKEMON_NATIONAL(CMARISA),
    EC_POKEMON_NATIONAL(MARISA),
    EC_WORD_ON,
    EC_WORD_ONCE,
    EC_POKEMON_NATIONAL(MEILING),
    EC_WORD_ONLY,
    EC_WORD_OOPS,
    EC_WORD_OPPONENT,
    EC_WORD_OPPOSITE,
    EC_WORD_OR,
    EC_WORD_OTHER,
    EC_WORD_OUR,
    EC_WORD_OUT,
    EC_MOVE(OUTRAGE),
    EC_WORD_OUTSIDE,
    EC_WORD_OVER,
    EC_WORD_OVERDO,
    EC_WORD_OVERGROW,
    EC_MOVE(OVERHEAT),
    EC_WORD_OVERWHELMING,
    EC_WORD_OWN,
    EC_WORD_OWN_TEMPO,
};

const u16 gEasyChatWordsByLetter_P[] = {
    EC_MOVE(PAIN_SPLIT),
    EC_POKEMON_NATIONAL(CIKU),
    EC_POKEMON_NATIONAL(IKU),
    EC_WORD_PARDON,
    EC_WORD_PARENT,
    EC_WORD_PARTNER,
    EC_WORD_PARTY,
    EC_WORD_PATTERN,
    EC_MOVE2(PAY_DAY),
    EC_WORD_PC,
    EC_MOVE2(PECK),
    EC_POKEMON(DREMILIA),
    EC_WORD_PERFECT,
    EC_WORD_PERFECTION,
    EC_MOVE2(PERISH_SONG),
    EC_POKEMON_NATIONAL(CHEN),
    EC_WORD_PERSON,
    EC_MOVE2(PETAL_DANCE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(AKOISHI),
    EC_POKEMON_NATIONAL(AKOISHI),
    EC_WORD_PHONE,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CLUIZE),
    EC_POKEMON_NATIONAL(CLUIZE),
    EC_WORD_PICKUP,
    EC_POKEMON_NATIONAL(KOAKUMA),
    EC_POKEMON_NATIONAL(CKOAKUMA),
    EC_POKEMON_NATIONAL(MEDICINE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CCIRNO),
    EC_POKEMON_NATIONAL(CCIRNO),
    EC_POKEMON_NATIONAL(DYOUMU),
    EC_MOVE2(PIN_MISSILE),
    EC_POKEMON_NATIONAL(DSUNNYMILK),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(SHOU),
    EC_POKEMON_NATIONAL(SHOU),
    EC_WORD_PKRS,
    EC_WORD_PLACE,
    EC_WORD_PLANS,
    EC_WORD_PLAY,
    EC_WORD_PLAYED,
    EC_WORD_PLAYS,
    EC_WORD_PLEASE,
    EC_WORD_PLUS,
    EC_WORD_PLUSH_DOLL,
    EC_POKEMON(TNAZRIN),
    EC_WORD_POINTS,
    EC_WORD_POISON,
    EC_MOVE2(POISON_FANG),
    EC_MOVE2(POISON_GAS),
    EC_WORD_POISON_POINT,
    EC_MOVE2(POISON_STING),
    EC_MOVE2(POISON_TAIL),
    EC_MOVE2(POISON_POWDER),
    EC_WORD_POKEBLOCK,
    EC_WORD_POKEDEX,
    EC_WORD_POKEMON,
    EC_WORD_POKENAV,
    EC_POKEMON_NATIONAL(CSHINKI),
    EC_POKEMON_NATIONAL(CHATATE),
    EC_POKEMON_NATIONAL(HATATE),
    EC_POKEMON_NATIONAL(CLETTY),
    EC_POKEMON_NATIONAL(KOGASA),
    EC_POKEMON(DYUYUKO),
    EC_WORD_POPULAR,
    EC_POKEMON_NATIONAL(REISENII),
    EC_POKEMON_NATIONAL(DBYAKUREN),
    EC_MOVE2(POUND),
    EC_MOVE(POWDER_SNOW),
    EC_WORD_POWER,
    EC_WORD_PRAISE,
    EC_WORD_PREPOSTEROUS,
    EC_MOVE2(PRESENT),
    EC_WORD_PRESSURE,
    EC_WORD_PRETEND,
    EC_WORD_PRETTY,
    EC_POKEMON_NATIONAL(KAGUYA),
    EC_WORD_PROBABLY,
    EC_WORD_PROMISE,
    EC_MOVE2(PROTECT),
    EC_MOVE(PSYBEAM),
    EC_MOVE(PSYCH_UP),
    DOUBLE_SPECIES_NAME
    EC_WORD_PSYCHIC,
    EC_MOVE(PSYCHIC),
    EC_MOVE(PSYCHO_BOOST),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CYORIHIME),
    EC_POKEMON_NATIONAL(CYORIHIME),
    EC_MOVE(PSYWAVE),
    EC_POKEMON_NATIONAL(ELIS),
    EC_WORD_PURE_POWER,
    EC_MOVE(PURSUIT),
    EC_WORD_PUSHOVER,
};

const u16 gEasyChatWordsByLetter_Q[] = {
    EC_POKEMON_NATIONAL(GENGETSU),
    EC_WORD_QUESTION,
    EC_MOVE2(QUICK_ATTACK),
    EC_POKEMON_NATIONAL(ELLY),
    EC_WORD_QUITE,
    EC_POKEMON_NATIONAL(DREISEN),
};

const u16 gEasyChatWordsByLetter_R[] = {
    EC_WORD_RADIO,
    EC_MOVE(RAGE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CIRNO),
    EC_POKEMON_NATIONAL(CIRNO),
    EC_POKEMON_NATIONAL(SHINGYOKUF),
    EC_MOVE(RAIN_DANCE),
    EC_WORD_RAIN_DISH,
    EC_POKEMON(TENMA),
    EC_WORD_RANK,
    EC_MOVE(RAPID_SPIN),
    EC_POKEMON_NATIONAL(CSATORI),
    EC_WORD_RARE,
    EC_WORD_RATHER,
    EC_POKEMON_NATIONAL(NAZRIN),
    EC_POKEMON_NATIONAL(CNAZRIN),
    EC_POKEMON(CMAMIZOU),
    EC_MOVE2(RAZOR_LEAF),
    EC_MOVE(RAZOR_WIND),
    EC_WORD_READY,
    EC_WORD_REALLY,
    EC_WORD_REAR,
    EC_WORD_RECEIVED,
    EC_WORD_RECOMMEND,
    EC_MOVE(RECOVER),
    EC_MOVE2(RECYCLE),
    EC_WORD_RED,
    EC_MOVE2(REFLECT),
    EC_MOVE2(REFRESH),
    EC_WORD_REFRESHING,
    EC_WORD_REFUSE,
    EC_POKEMON(CFUTO),
    EC_POKEMON(TOJIKO),
    EC_POKEMON(FUTO),
    EC_WORD_REJECT,
    EC_WORD_RELEASE,
    EC_POKEMON(ADLETTY),
    EC_POKEMON_NATIONAL(AYUUKA),
    EC_WORD_RENTAL,
    EC_MOVE2(REST),
    EC_WORD_RESUSCITATE,
    EC_MOVE(RETURN),
    EC_MOVE2(REVENGE),
    EC_MOVE(REVERSAL),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CICHIRIN),
    EC_POKEMON_NATIONAL(CICHIRIN),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(UTSUHO),
    EC_POKEMON_NATIONAL(UTSUHO),
    EC_WORD_RIBBON,
    EC_WORD_RICKETY,
    EC_WORD_RIGHT,
    EC_WORD_RIPPED,
    EC_WORD_RIVAL,
    EC_MOVE2(ROAR),
    EC_WORD_ROCK,
    EC_MOVE2(ROCK_BLAST),
    EC_WORD_ROCK_HEAD,
    EC_MOVE(ROCK_SLIDE),
    EC_MOVE(ROCK_SMASH),
    EC_MOVE(ROCK_THROW),
    EC_MOVE(ROCK_TOMB),
    EC_WORD_ROCK_SOLID,
    EC_MOVE2(ROLE_PLAY),
    EC_MOVE2(ROLLING_KICK),
    EC_MOVE(ROLLOUT),
    EC_WORD_ROMANTIC,
    EC_WORD_ROOM,
    EC_POKEMON(ADREISEN),
    EC_WORD_ROUGH_SKIN,
    EC_WORD_RPG,
    EC_WORD_RUBY,
    EC_WORD_RULE,
    EC_WORD_RUN,
    EC_WORD_RUN_AWAY,
    EC_WORD_RUNS,
};

const u16 gEasyChatWordsByLetter_S[] = {
    EC_POKEMON(ADMYSTIA),
    EC_MOVE(SACRED_FIRE),
    EC_WORD_SAD,
    EC_WORD_SADLY,
    EC_MOVE(SAFEGUARD),
    EC_WORD_SAID,
    EC_POKEMON(YOSHIKA),
    EC_WORD_SAND_STREAM,
    EC_MOVE(SAND_TOMB),
    EC_WORD_SAND_VEIL,
    EC_MOVE(SAND_ATTACK),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CRUMIA),
    EC_POKEMON_NATIONAL(CRUMIA),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(RUMIA),
    EC_POKEMON_NATIONAL(RUMIA),
    EC_MOVE(SANDSTORM),
    EC_WORD_SAPPHIRE,
    EC_WORD_SATISFIED,
    EC_WORD_SATURDAY,
    EC_WORD_SAYS,
    EC_WORD_SCARY,
    EC_MOVE(SCARY_FACE),
    EC_WORD_SCATTER,
    EC_POKEMON(TYUKARI),
    EC_WORD_SCHOOL,
    EC_POKEMON_NATIONAL(TREISEN),
    EC_MOVE2(SCRATCH),
    EC_MOVE(SCREECH),
    EC_POKEMON_NATIONAL(AYA),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(LUNACHILD),
    EC_POKEMON_NATIONAL(LUNACHILD),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(STARSAPPHIRE),
    EC_POKEMON_NATIONAL(STARSAPPHIRE),
    EC_POKEMON(HLILYWHITE),
    EC_WORD_SEARCH,
    EC_WORD_SECRET,
    EC_WORD_SECRET_BASE,
    EC_MOVE2(SECRET_POWER),
    EC_WORD_SEE,
    EC_WORD_SEE_YA,
    EC_POKEMON(SRIN),
    EC_WORD_SEEK,
    EC_POKEMON_NATIONAL(CNITORI),
    EC_WORD_SEEMS,
    EC_WORD_SEES,
    EC_MOVE2(SEISMIC_TOSS),
    EC_MOVE(SELF_DESTRUCT),
    EC_WORD_SENSE,
    EC_POKEMON_NATIONAL(CHIYURI),
    EC_WORD_SERENE_GRACE,
    EC_WORD_SERIOUS,
    EC_WORD_SERIOUSLY,
    EC_WORD_SERVICE,
    EC_POKEMON(TSATORI),
    EC_MOVE(SHADOW_BALL),
    EC_MOVE(SHADOW_PUNCH),
    EC_WORD_SHADOW_TAG,
    EC_WORD_SHADY,
    EC_WORD_SHAKE,
    EC_WORD_SHAKY,
    EC_WORD_SHALLOW,
    EC_POKEMON(DSHIKIEIKI),
    EC_MOVE(SHARPEN),
    EC_WORD_SHE,
    EC_WORD_SHE_IS,
    EC_WORD_SHE_WAS,
    EC_WORD_SHED_SKIN,
    EC_POKEMON(ASHOU),
    EC_MOVE(SHEER_COLD),
    EC_POKEMON(CYOSHIKA),
    EC_WORD_SHELL_ARMOR,
    EC_POKEMON_NATIONAL(CKOMACHI),
    EC_WORD_SHIELD_DUST,
    EC_POKEMON(ZOMBIEFAIRY),
    EC_WORD_SHINE,
    EC_MOVE2(SHOCK_WAVE),
    EC_WORD_SHOCKED,
    EC_WORD_SHOPPING,
    EC_WORD_SHOULD,
    EC_WORD_SHOW,
    EC_WORD_SHREDDED,
    EC_POKEMON(DSUWAKO),
    EC_POKEMON_NATIONAL(TSAKUYA),
    EC_POKEMON(DDAIYOUSEI),
    EC_WORD_SIBLINGS,
    EC_WORD_SIGH,
    EC_MOVE(SIGNAL_BEAM),
    EC_POKEMON(ARAN),
    EC_WORD_SILKY,
    EC_WORD_SILVER,
    EC_MOVE(SILVER_WIND),
    EC_WORD_SIMPLE,
    EC_WORD_SINCE,
    EC_MOVE(SING),
    EC_WORD_SINK,
    EC_WORD_SISTER,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(ATENSHI),
    EC_POKEMON_NATIONAL(ATENSHI),
    EC_MOVE(SKETCH),
    EC_WORD_SKILL,
    EC_MOVE(SKILL_SWAP),
    EC_WORD_SKILLED,
    EC_POKEMON_NATIONAL(MIMICHAN),
    EC_POKEMON(ASUNNYMILK),
    EC_MOVE2(SKULL_BASH),
    EC_MOVE(SKY_ATTACK),
    EC_MOVE(SKY_UPPERCUT),
    EC_MOVE2(SLACK_OFF),
    EC_POKEMON(CMIMA),
    EC_POKEMON(SSARIEL),
    EC_MOVE2(SLAM),
    EC_MOVE(SLASH),
    EC_WORD_SLEEP,
    EC_MOVE2(SLEEP_POWDER),
    EC_MOVE2(SLEEP_TALK),
    EC_WORD_SLEPT,
    EC_WORD_SLIDE,
    EC_WORD_SLIMY,
    EC_POKEMON_NATIONAL(CEIRIN),
    EC_POKEMON_NATIONAL(RIKAKO),
    EC_POKEMON_NATIONAL(SATORI),
    EC_MOVE2(SLUDGE),
    EC_MOVE2(SLUDGE_BOMB),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(AMYSTIA),
    EC_POKEMON_NATIONAL(AMYSTIA),
    EC_WORD_SMACK,
    EC_WORD_SMALL,
    EC_WORD_SMART,
    EC_WORD_SMARTNESS,
    EC_POKEMON_NATIONAL(DICHIRIN),
    EC_WORD_SMELL,
    EC_WORD_SMELL_YA,
    EC_MOVE(SMELLING_SALT),
    EC_WORD_SMITE,
    EC_MOVE(SMOG),
    EC_MOVE(SMOKESCREEN),
    EC_POKEMON_NATIONAL(DKANAKO),
    EC_WORD_SMOOTH,
    EC_MOVE2(SNATCH),
    EC_POKEMON_NATIONAL(ARUMIA),
    EC_MOVE(SNORE),
    EC_POKEMON_NATIONAL(BYAKUREN),
    EC_WORD_SNORT,
    EC_POKEMON(DPATCHOULI),
    EC_POKEMON_NATIONAL(AMOKOU),
    EC_WORD_SO,
    EC_MOVE2(SOFT_BOILED),
    EC_WORD_SOFTWARE,
    EC_MOVE(SOLAR_BEAM),
    EC_POKEMON(SYORIHIME),
    EC_WORD_SOME,
    EC_WORD_SOMEHOW,
    EC_WORD_SOMEONE,
    EC_WORD_SOMETHING,
    EC_WORD_SOMETIME,
    EC_WORD_SON,
    EC_WORD_SONG,
    EC_WORD_SONGS,
    EC_MOVE(SONIC_BOOM),
    EC_WORD_SOON,
    EC_WORD_SORRY,
    EC_WORD_SOUNDPROOF,
    EC_WORD_SP_ABILITY,
    EC_MOVE(SPARK),
    EC_POKEMON_NATIONAL(CTOKIKO),
    EC_WORD_SPECTATOR,
    EC_WORD_SPEED_BOOST,
    EC_POKEMON(ASUIKA),
    EC_MOVE(SPIDER_WEB),
    EC_MOVE2(SPIKE_CANNON),
    EC_MOVE2(SPIKES),
    EC_POKEMON_NATIONAL(KAZAMI),
    EC_POKEMON(SKOGASA),
    EC_WORD_SPIRALING,
    EC_WORD_SPIRIT,
    EC_MOVE2(SPIT_UP),
    EC_MOVE(SPITE),
    EC_MOVE2(SPLASH),
    EC_POKEMON(ALILYBLACK),
    EC_MOVE(SPORE),
    EC_WORD_SPORTS,
    EC_WORD_SPRING,
    EC_POKEMON_NATIONAL(CLILYWHITE),
    EC_WORD_STAGE,
    EC_POKEMON_NATIONAL(TBYAKUREN),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(YUUKA),
    EC_POKEMON_NATIONAL(YUUKA),
    EC_WORD_START,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CYUUKA),
    EC_POKEMON_NATIONAL(CYUUKA),
    EC_WORD_STATIC,
    EC_WORD_STAY_AT_HOME,
    EC_WORD_STEEL,
    EC_MOVE2(STEEL_WING),
    EC_POKEMON_NATIONAL(TNITORI),
    EC_WORD_STENCH,
    EC_WORD_STICKY_HOLD,
    EC_MOVE2(STOCKPILE),
    EC_MOVE2(STOMP),
    EC_WORD_STOP,
    EC_WORD_STORE,
    EC_WORD_STORES,
    EC_WORD_STORY,
    EC_WORD_STRATEGY,
    EC_MOVE(STRENGTH),
    EC_MOVE(STRING_SHOT),
    EC_WORD_STRONG,
    EC_MOVE2(STRUGGLE),
    EC_WORD_STUDY,
    EC_MOVE(STUN_SPORE),
    EC_WORD_STURDY,
    EC_MOVE(SUBMISSION),
    EC_MOVE2(SUBSTITUTE),
    EC_WORD_SUCTION_CUPS,
    EC_POKEMON_NATIONAL(YUMEKO),
    EC_POKEMON_NATIONAL(SUTSUHO),
    EC_WORD_SUMMER,
    EC_WORD_SUNDAY,
    EC_POKEMON_NATIONAL(CMUGETSU),
    EC_POKEMON_NATIONAL(RIKA),
    EC_MOVE2(SUNNY_DAY),
    EC_WORD_SUPER,
    EC_MOVE(SUPER_FANG),
    EC_MOVE2(SUPERPOWER),
    EC_MOVE2(SUPERSONIC),
    EC_MOVE2(SURF),
    EC_WORD_SURPRISE,
    EC_WORD_SURRENDER,
    EC_POKEMON(SFLANDRE),
    EC_POKEMON(DIKU),
    EC_MOVE(SWAGGER),
    EC_MOVE2(SWALLOW),
    EC_POKEMON(ADMEILING),
    EC_POKEMON(TCIRNO),
    EC_WORD_SWARM,
    EC_MOVE2(SWEET_KISS),
    EC_MOVE(SWEET_SCENT),
    EC_WORD_SWEETS,
    EC_POKEMON(ATOKIKO),
    EC_MOVE(SWIFT),
    EC_WORD_SWIFT_SWIM,
    EC_POKEMON_NATIONAL(TCHEN),
    EC_MOVE2(SWORDS_DANCE),
    EC_WORD_SYNCHRONIZE,
    EC_MOVE(SYNTHESIS),
    EC_WORD_SYSTEM,
};

const u16 gEasyChatWordsByLetter_T[] = {
    EC_MOVE2(TACKLE),
    EC_MOVE2(TAIL_GLOW),
    EC_MOVE(TAIL_WHIP),
    EC_POKEMON(TSHOU),
    EC_WORD_TAKE,
    EC_MOVE2(TAKE_DOWN),
    EC_WORD_TAKE_THAT,
    EC_WORD_TALENT,
    EC_WORD_TALK,
    EC_WORD_TALKING,
    EC_POKEMON_NATIONAL(CSUNNYMILK),
    EC_WORD_TASTY,
    EC_MOVE2(TAUNT),
    EC_POKEMON_NATIONAL(CWRIGGLE),
    EC_WORD_TCH,
    EC_WORD_TEACH,
    EC_WORD_TEACHER,
    EC_WORD_TEACHES,
    EC_POKEMON_NATIONAL(SRUMIA),
    EC_MOVE2(TEETER_DANCE),
    EC_MOVE2(TELEPORT),
    EC_WORD_TELEVISION,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CMURASA),
    EC_POKEMON_NATIONAL(CMURASA),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(MURASA),
    EC_POKEMON_NATIONAL(MURASA),
    EC_WORD_TERRIBLE,
    EC_WORD_TEST,
    EC_WORD_THAN,
    EC_WORD_THANK_YOU,
    EC_WORD_THANKS,
    EC_WORD_THAT,
    EC_WORD_THAT_WAS,
    EC_WORD_THAT_S,
    EC_WORD_THAT_S_IT_EXCL,
    EC_WORD_THE,
    EC_WORD_THEIR,
    EC_WORD_THERE,
    EC_WORD_THESE,
    EC_WORD_THESE_WERE,
    EC_WORD_THEY,
    EC_WORD_THEY_WERE,
    EC_WORD_THEY_RE,
    EC_WORD_THEY_VE,
    EC_WORD_THICK,
    EC_WORD_THICK_FAT,
    EC_MOVE2(THIEF),
    EC_WORD_THIN,
    EC_WORD_THING,
    EC_WORD_THINGS,
    EC_WORD_THINK,
    EC_WORD_THINKS,
    EC_WORD_THIRSTY,
    EC_WORD_THIS,
    EC_WORD_THIS_IS_IT_EXCL,
    EC_WORD_THOSE,
    EC_WORD_THOSE_ARE,
    EC_WORD_THOSE_WERE,
    EC_MOVE(THRASH),
    EC_WORD_THROW,
    EC_MOVE(THUNDER),
    EC_MOVE2(THUNDER_WAVE),
    EC_MOVE(THUNDERBOLT),
    EC_MOVE(THUNDER_PUNCH),
    EC_MOVE2(THUNDER_SHOCK),
    EC_WORD_THURSDAY,
    EC_MOVE(TICKLE),
    EC_WORD_TICKLISH,
    EC_WORD_TIGHT,
    EC_WORD_TIME,
    EC_WORD_TIMES,
    EC_WORD_TIRED,
    EC_WORD_TO,
    EC_WORD_TO_HER,
    EC_WORD_TO_ME,
    EC_WORD_TO_THEM,
    EC_WORD_TO_US,
    EC_WORD_TO_WHOM,
    EC_WORD_TODAY,
    EC_POKEMON_NATIONAL(MARGATROID),
    EC_WORD_TOGETHER,
    EC_POKEMON_NATIONAL(CKANA),
    EC_WORD_TOMORROW,
    EC_WORD_TOO,
    EC_WORD_TOO_LATE,
    EC_WORD_TOO_STRONG,
    EC_WORD_TOO_WEAK,
    EC_WORD_TOP,
    EC_POKEMON(AALICE),
    EC_POKEMON(CSARIEL),
    EC_MOVE(TORMENT),
    EC_WORD_TORRENT,
    EC_WORD_TOTALLY,
    EC_POKEMON_NATIONAL(ORANGE),
    EC_WORD_TOUGH,
    EC_WORD_TOUGHNESS,
    EC_WORD_TOURNAMENT,
    EC_WORD_TOWER,
    EC_MOVE2(TOXIC),
    EC_WORD_TOYS,
    EC_WORD_TRACE,
    EC_WORD_TRADE,
    EC_WORD_TRAIN,
    EC_WORD_TRAINER,
    EC_WORD_TRAINS,
    EC_MOVE2(TRANSFORM),
    EC_POKEMON(DMINORIKO),
    EC_WORD_TRAVEL,
    EC_WORD_TREASURE,
    EC_POKEMON(HEIRIN),
    EC_WORD_TRENDY,
    EC_MOVE2(TRI_ATTACK),
    EC_MOVE2(TRICK),
    EC_WORD_TRIES,
    EC_MOVE2(TRIPLE_KICK),
    EC_POKEMON(APATCHOULI),
    EC_WORD_TRUANT,
    EC_WORD_TRULY,
    EC_WORD_TRUMP_CARD,
    EC_WORD_TRUST,
    EC_WORD_TRY,
    EC_WORD_TUESDAY,
    EC_MOVE2(TWINEEDLE),
    EC_WORD_TWIRLING,
    EC_MOVE2(TWISTER),
    EC_WORD_TYPE,
    EC_POKEMON_NATIONAL(CORANGE),
    EC_POKEMON_NATIONAL(MIMA),
    EC_POKEMON_NATIONAL(TICHIRIN),
};

const u16 gEasyChatWordsByLetter_U[] = {
    EC_WORD_UGLY,
    EC_WORD_UH_HUH,
    EC_WORD_UH_OH,
    EC_WORD_UM,
    EC_POKEMON_NATIONAL(KURUMI),
    EC_WORD_UNAVOIDABLE,
    EC_WORD_UNBELIEVABLE,
    EC_WORD_UNCLE,
    EC_WORD_UNDERSTAND,
    EC_WORD_UNDERSTANDS,
    EC_WORD_UNDERSTOOD,
    EC_WORD_UNION,
    EC_WORD_UNTIL,
    EC_WORD_UP,
    EC_WORD_UPBEAT,
    EC_MOVE(UPROAR),
    EC_WORD_UPSIDE_DOWN,
    EC_WORD_URGH,
    EC_POKEMON_NATIONAL(HMYSTIA),
    EC_WORD_USE,
    EC_WORD_USELESS,
    EC_WORD_USES,
    EC_WORD_USING,
};

const u16 gEasyChatWordsByLetter_V[] = {
    EC_WORD_VACATION,
    EC_POKEMON_NATIONAL(NUE),
    EC_POKEMON_NATIONAL(KOISHI),
    EC_POKEMON_NATIONAL(CKOISHI),
    EC_POKEMON_NATIONAL(CALICE),
    EC_WORD_VERSION,
    EC_WORD_VERSUS,
    EC_WORD_VERY,
    EC_POKEMON(TSUIKA),
    EC_MOVE2(VICE_GRIP),
    EC_WORD_VICTORY,
    EC_POKEMON_NATIONAL(PARSEE),
    EC_WORD_VIEWING,
    EC_POKEMON(ALILYWHITE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(AKEINE),
    EC_POKEMON_NATIONAL(AKEINE),
    EC_MOVE2(VINE_WHIP),
    EC_WORD_VITAL_SPIRIT,
    EC_MOVE(VITAL_THROW),
    EC_POKEMON(YUKI),
    EC_WORD_VOLT_ABSORB,
    EC_MOVE2(VOLT_TACKLE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CMERLIN),
    EC_POKEMON_NATIONAL(CMERLIN),
    EC_WORD_VORACIOUS,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CSUIKA),
    EC_POKEMON_NATIONAL(CSUIKA),
};

const u16 gEasyChatWordsByLetter_W[] = {
    EC_WORD_WAAAH,
    EC_WORD_WAHAHAHA,
    EC_POKEMON(DMEILING),
    EC_POKEMON(TENSOKU),
    EC_WORD_WAIT,
    EC_WORD_WAKE_UP,
    EC_WORD_WAKES_UP,
    EC_WORD_WALK,
    EC_WORD_WALKING,
    EC_WORD_WALKS,
    EC_POKEMON(SLILYBLACK),
    EC_WORD_WANDERING,
    EC_WORD_WANNABE,
    EC_WORD_WANT,
    EC_WORD_WANTS,
    EC_POKEMON_NATIONAL(LILYWHITE),
    EC_WORD_WAS,
    EC_WORD_WASN_T,
    EC_WORD_WATER,
    EC_WORD_WATER_ABSORB,
    EC_MOVE2(WATER_GUN),
    EC_MOVE2(WATER_PULSE),
    EC_MOVE2(WATER_SPORT),
    EC_MOVE(WATER_SPOUT),
    EC_WORD_WATER_VEIL,
    EC_MOVE2(WATERFALL),
    EC_WORD_WAY,
    EC_WORD_WE,
    EC_WORD_WE_RE,
    EC_WORD_WE_VE,
    EC_WORD_WEAK,
    EC_WORD_WEAKENED,
    EC_MOVE(WEATHER_BALL),
    EC_WORD_WEDNESDAY,
    EC_POKEMON_NATIONAL(CSHIZUHA),
    EC_WORD_WEEK,
    EC_POKEMON_NATIONAL(CPARSEE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CUTSUHO),
    EC_POKEMON_NATIONAL(CUTSUHO),
    EC_WORD_WEIRD,
    EC_WORD_WELCOME,
    EC_WORD_WELL,
    EC_WORD_WELL_THEN,
    EC_WORD_WENT,
    EC_WORD_WERE,
    EC_WORD_WEREN_T,
    EC_WORD_WHAT,
    EC_WORD_WHAT_S_UP_QUES,
    EC_WORD_WHEN,
    EC_WORD_WHEN_I_WIN,
    EC_WORD_WHERE,
    EC_WORD_WHICH,
    EC_WORD_WHILE,
    EC_MOVE(WHIRLPOOL),
    EC_MOVE2(WHIRLWIND),
    EC_POKEMON(DHATATE),
    EC_POKEMON(SMARISA),
    EC_WORD_WHITE_SMOKE,
    EC_WORD_WHO,
    EC_WORD_WHO_IS,
    EC_WORD_WHO_WAS,
    EC_WORD_WHOAH,
    EC_WORD_WHOM,
    EC_WORD_WHOSE,
    EC_WORD_WHY,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(MYSTIA),
    EC_POKEMON_NATIONAL(MYSTIA),
    EC_WORD_WILD,
    EC_WORD_WILL,
    EC_WORD_WILL_BE_HERE,
    EC_MOVE(WILL_O_WISP),
    EC_WORD_WIMPY,
    EC_WORD_WIN,
    EC_MOVE2(WING_ATTACK),
    EC_POKEMON(TKOGASA),
    EC_WORD_WINNER,
    EC_WORD_WINS,
    EC_WORD_WINTER,
    EC_WORD_WIRELESS,
    EC_MOVE2(WISH),
    EC_WORD_WITH,
    EC_MOVE(WITHDRAW),
    EC_WORD_WITHOUT,
    EC_WORD_WOBBLY,
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CMAI),
    EC_POKEMON_NATIONAL(CMAI),
    EC_WORD_WOMAN,
    EC_WORD_WON,
    EC_WORD_WON_T,
    EC_WORD_WONDER,
    EC_WORD_WONDER_GUARD,
    EC_POKEMON_NATIONAL(CGENGETSU),
    EC_WORD_WORD,
    EC_WORD_WORK,
    EC_WORD_WORKING,
    EC_WORD_WORKS,
    EC_WORD_WORLD,
    EC_WORD_WORRY,
    EC_WORD_WOULD,
    EC_WORD_WOW,
    EC_WORD_WOWEE,
    EC_MOVE2(WRAP),
    EC_WORD_WROOOAAR_EXCL,
    EC_POKEMON(HLYRICA),
    EC_POKEMON(AMINORIKO),
};

const u16 gEasyChatWordsByLetter_X[] = {
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CHAKUREI),
    EC_POKEMON_NATIONAL(CHAKUREI),
};

const u16 gEasyChatWordsByLetter_Y[] = {
    EC_WORD_YAHOO,
    EC_POKEMON_NATIONAL(MUGETSU),
    EC_MOVE(YAWN),
    EC_WORD_YAY,
    EC_WORD_YEAH,
    EC_WORD_YEAH_YEAH,
    EC_WORD_YEEHAW_EXCL,
    EC_WORD_YES,
    EC_WORD_YES_SIR_EXCL,
    EC_WORD_YESTERDAY,
    EC_WORD_YET,
    EC_WORD_YO,
    EC_WORD_YOU,
    EC_WORD_YOU_RE,
    EC_WORD_YOU_VE,
    EC_WORD_YOUNG,
    EC_WORD_YOUR,
    EC_WORD_YOURS,
    EC_WORD_YUP,
};

const u16 gEasyChatWordsByLetter_Z[] = {
    EC_POKEMON(DSATORI),
    EC_MOVE2(ZAP_CANNON),
    EC_POKEMON_NATIONAL(KIKURI),
    EC_POKEMON(HLUNASA),
    DOUBLE_SPECIES_NAME
    EC_POKEMON(CMINORIKO),
    EC_POKEMON_NATIONAL(CMINORIKO),
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseHi[] = {
    EC_POKEMON_NATIONAL(LUIZE),
    EC_WORD_AIYEEH,
    EC_POKEMON_NATIONAL(CSHIZUHA),
    EC_WORD_HERO,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CCIRNO),
    EC_POKEMON(CCIRNO),
    EC_MOVE2(LIGHT_SCREEN),
    EC_WORD_SHINE,
    EC_WORD_LOW,
    EC_POKEMON_NATIONAL(YUKARI),
    EC_WORD_FLYING,
    EC_POKEMON_NATIONAL(KOAKUMA),
    EC_POKEMON_NATIONAL(CKOAKUMA),
    EC_WORD_LEFT,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CLUIZE),
    EC_POKEMON(CLUIZE),
    EC_MOVE2(SCRATCH),
    EC_WORD_SURPRISE,
    EC_MOVE2(HYPER_FANG),
    EC_POKEMON_NATIONAL(CYUKARI),
    EC_WORD_DROUGHT,
    EC_WORD_HIDDEN,
    EC_WORD_PERSON,
    EC_WORD_AWFUL,
    EC_POKEMON_NATIONAL(ALICE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CYUUKA),
    EC_POKEMON(CYUUKA),
    EC_WORD_ALONE,
    EC_POKEMON_NATIONAL(CELLY),
    EC_MOVE2(EMBER),
    EC_POKEMON(TSUIKA),
    EC_WORD_BORED,
    EC_POKEMON_NATIONAL(RIKA),
    EC_WORD_SECRET,
    EC_WORD_SECRET_BASE,
    EC_MOVE2(SECRET_POWER),
    EC_POKEMON_NATIONAL(SRUMIA),
    EC_WORD_HIYAH,
    EC_MOVE2(DIZZY_PUNCH),
    EC_WORD_LIGHTNINGROD,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CMERLIN),
    EC_POKEMON(CMERLIN),
    EC_WORD_DAYTIME,
    EC_MOVE2(BULK_UP),
    EC_WORD_NAP,
    EC_WORD_HEROINE,
    EC_POKEMON(AREMILIA),
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseFu[] = {
    EC_WORD_FIGHT,
    EC_POKEMON_NATIONAL(YUUGENMAGAN),
    EC_WORD_FASHION,
    EC_WORD_FEAR,
    EC_WORD_FEVER,
    EC_MOVE2(IMPRISON),
    EC_POKEMON_NATIONAL(SUWAKO),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(PATCHOULI),
    EC_POKEMON(PATCHOULI),
    EC_POKEMON_NATIONAL(CSHOU),
    EC_POKEMON(DYUUGI),
    EC_MOVE2(FEATHER_DANCE),
    EC_POKEMON_NATIONAL(TLUNACHILD),
    EC_MOVE2(WHIRLWIND),
    EC_WORD_COMPOUNDEYES,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(MYSTIA),
    EC_POKEMON(MYSTIA),
    EC_MOVE2(BEAT_UP),
    EC_WORD_MYSTERY,
    EC_POKEMON_NATIONAL(SANAE),
    EC_POKEMON_NATIONAL(CSANAE),
    EC_WORD_MARVEL_SCALE,
    EC_WORD_WONDER_GUARD,
    EC_POKEMON_NATIONAL(CALICE),
    EC_WORD_LACKS,
    EC_WORD_RESUSCITATE,
    EC_WORD_SMACK,
    EC_WORD_FUFUFU,
    EC_POKEMON_NATIONAL(CBYAKUREN),
    EC_WORD_WIMPY,
    EC_POKEMON_NATIONAL(DKAGUYA),
    EC_WORD_LOL,
    EC_MOVE2(BLIZZARD),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CMARGATROID),
    EC_POKEMON(CMARGATROID),
    EC_WORD_SNORT,
    EC_MOVE2(STOMP),
    EC_WORD_INSOMNIA,
    EC_WORD_WINTER,
    EC_WORD_LEVITATE,
    EC_POKEMON(ASANAE),
    EC_WORD_PLUS,
    EC_MOVE2(BLAST_BURN),
    EC_POKEMON(TNAZRIN),
    EC_POKEMON_NATIONAL(KURUMI),
    EC_MOVE2(FLASH),
    EC_WORD_WOBBLY,
    EC_MOVE2(TEETER_DANCE),
    EC_WORD_PRETEND,
    EC_POKEMON_NATIONAL(KONNGARA),
    EC_WORD_WANNABE,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CMYSTIA),
    EC_POKEMON(CMYSTIA),
    EC_POKEMON_NATIONAL(AMOKOU),
    EC_WORD_SHAKY,
    EC_MOVE2(CRUSH_CLAW),
    EC_MOVE2(BLAZE_KICK),
    EC_MOVE2(PRESENT),
    EC_WORD_PRESSURE,
    EC_WORD_BLEND,
    EC_WORD_FRONTIER,
    EC_WORD_HUMPH,
    EC_MOVE2(ERUPTION),
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseHe[] = {
    EC_WORD_HEY,
    EC_POKEMON(DLAYLA),
    EC_WORD_BABY,
    EC_POKEMON_NATIONAL(TORI),
    EC_WORD_BEST,
    EC_WORD_LOUSY,
    EC_WORD_HEHEHE,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CHINA),
    EC_POKEMON(CHINA),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(HINA),
    EC_POKEMON(HINA),
    EC_WORD_LIQUID_OOZE,
    EC_MOVE2(SLUDGE),
    EC_MOVE2(SLUDGE_BOMB),
    EC_MOVE2(GLARE),
    EC_WORD_HEH,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(HSAKUYA),
    EC_POKEMON(HSAKUYA),
    EC_POKEMON(DREMILIA),
    EC_POKEMON_NATIONAL(DPARSEE),
    EC_POKEMON_NATIONAL(CHEN),
    EC_WORD_RIPPED,
    EC_POKEMON_NATIONAL(CRIN),
    EC_WORD_STUDY,
    EC_WORD_COLOR_CHANGE,
    EC_MOVE2(TRANSFORM),
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseHo[] = {
    EC_MOVE2(POISON_TAIL),
    EC_WORD_POINTS,
    EC_POKEMON_NATIONAL(AKOMACHI),
    EC_WORD_SOUNDPROOF,
    EC_WORD_ADVENTURE,
    EC_WORD_EFFECT_SPORE,
    EC_MOVE2(ROAR),
    EC_POKEMON(TENSOKU),
    EC_POKEMON(DMEILING),
    EC_WORD_DUDE,
    EC_WORD_BOARD,
    EC_POKEMON_NATIONAL(YUMEMI),
    EC_POKEMON(YOSHIKA),
    EC_WORD_BALL,
    EC_MOVE2(BONE_RUSH),
    EC_WORD_THEY,
    EC_WORD_THEY_WERE,
    EC_WORD_TO_THEM,
    EC_WORD_THEIR,
    EC_WORD_THEY_RE,
    EC_WORD_THEY_VE,
    EC_WORD_POKENAV,
    EC_WORD_POKEMON,
    EC_WORD_GET,
    EC_WORD_POKEDEX,
    EC_WORD_PKRS,
    EC_MOVE2(CAMOUFLAGE),
    EC_WORD_WANT,
    EC_MOVE2(COVET),
    EC_POKEMON(ADALICE),
    EC_MOVE2(TAIL_GLOW),
    EC_POKEMON(DYUYUKO),
    EC_POKEMON_NATIONAL(MEDICINE),
    EC_WORD_AS_MUCH_AS,
    EC_POKEMON_NATIONAL(KOGASA),
    EC_MOVE2(BONE_CLUB),
    EC_MOVE2(BONEMERANG),
    EC_WORD_FIRE,
    EC_MOVE2(FIRE_SPIN),
    EC_WORD_FLAME_BODY,
    EC_MOVE2(FIRE_PUNCH),
    EC_POKEMON_NATIONAL(MIMICHAN),
    EC_WORD_HOHOHO,
    EC_WORD_PRAISE,
    EC_WORD_UH_HUH,
    EC_POKEMON_NATIONAL(REISENII),
    EC_POKEMON_NATIONAL(DBYAKUREN),
    EC_MOVE2(VOLT_TACKLE),
    EC_WORD_POKEBLOCK,
    EC_MOVE2(PERISH_SONG),
    EC_WORD_SHREDDED,
    EC_POKEMON(TSANAE),
    EC_WORD_BOOK,
    EC_WORD_SERIOUS,
    EC_WORD_REALLY,
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseMa[] = {
    EC_WORD_OH_DEAR,
    EC_WORD_IN,
    EC_WORD_GIVE_UP,
    EC_WORD_MINUS,
    EC_POKEMON(HNAZRIN),
    EC_WORD_DAILY,
    EC_WORD_OWN_TEMPO,
    EC_MOVE2(WRAP),
    EC_MOVE2(SPIKES),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(ACHEN),
    EC_POKEMON(ACHEN),
    EC_MOVE2(MAGNITUDE),
    EC_POKEMON(AFLANDRE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(AMYSTIA),
    EC_POKEMON(AMYSTIA),
    EC_WORD_MAGMA_ARMOR,
    EC_POKEMON_NATIONAL(ELLY),
    EC_WORD_OVERDO,
    EC_WORD_LOSS,
    EC_WORD_IF_I_LOSE,
    EC_WORD_LOST,
    EC_WORD_LOSE,
    EC_WORD_TRULY,
    EC_WORD_SERIOUSLY,
    EC_MOVE2(MAGICAL_LEAF),
    EC_MOVE2(MAGIC_COAT),
    EC_WORD_MACHINE,
    EC_WORD_OUT,
    EC_WORD_MASTER,
    EC_WORD_INCREASING,
    EC_WORD_TOO,
    EC_WORD_LIKE,
    EC_POKEMON_NATIONAL(TENSHI),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CUTSUHO),
    EC_POKEMON(CUTSUHO),
    EC_WORD_SMELL,
    EC_WORD_YET,
    EC_WORD_MISTAKE,
    EC_POKEMON(HMERLIN),
    EC_WORD_TOTALLY,
    EC_MOVE2(MUD_SHOT),
    EC_MOVE2(MACH_PUNCH),
    EC_WORD_FESTIVAL,
    EC_WORD_UNTIL,
    EC_WORD_WAIT,
    EC_WORD_GUARD,
    EC_MOVE2(PROTECT),
    EC_POKEMON(AMEDICINE),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(KOTOHIME),
    EC_POKEMON(KOTOHIME),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CYUMEKO),
    EC_POKEMON(CYUMEKO),
    EC_MOVE2(DEFENSE_CURL),
    EC_WORD_AS_IF,
    EC_POKEMON(ADMEILING),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(MERLIN),
    EC_POKEMON(MERLIN),
    EC_MOVE2(ROLLING_KICK),
    EC_WORD_COMICS,
    EC_POKEMON_NATIONAL(CKAGUYA),
    EC_WORD_SATISFIED,
    EC_POKEMON_NATIONAL(TAYA),
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseMi[] = {
    EC_WORD_PARTNER,
    EC_MOVE2(SUBSTITUTE),
    EC_WORD_RIGHT,
    EC_MOVE2(DETECT),
    EC_MOVE2(PIN_MISSILE),
    EC_WORD_WATER,
    EC_MOVE2(WATER_SPORT),
    EC_POKEMON(DREIMU),
    EC_MOVE2(WATER_GUN),
    EC_MOVE2(MIST_BALL),
    EC_MOVE2(WATER_PULSE),
    EC_WORD_WATER_VEIL,
    EC_WORD_SHOW,
    EC_WORD_SEE,
    EC_WORD_DID,
    EC_MOVE2(FURY_ATTACK),
    EC_MOVE2(FURY_SWIPES),
    EC_MOVE2(DESTINY_BOND),
    EC_WORD_LOOKS,
    EC_WORD_REJECT,
    EC_WORD_ACCEPT,
    EC_POKEMON_NATIONAL(CTOYOHIME),
    EC_MOVE2(FALSE_SWIPE),
    EC_MOVE2(FORESIGHT),
    EC_POKEMON_NATIONAL(AKYUU),
    EC_POKEMON_NATIONAL(SARIEL),
    EC_MOVE2(MIRROR_COAT),
    EC_MOVE2(FUTURE_SIGHT),
    EC_WORD_SEES,
    EC_MOVE2(MILK_DRINK),
    EC_POKEMON_NATIONAL(SHINGYOKUO),
    EC_POKEMON(DTEWI),
    EC_WORD_WE,
    EC_WORD_BEEN,
    EC_WORD_TO_US,
    EC_WORD_OUR,
    EC_WORD_WE_RE,
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseMu[] = {
    EC_WORD_MOOD,
    EC_POKEMON_NATIONAL(CYUKI),
    EC_WORD_OLDEN,
    EC_WORD_ARRGH,
    EC_WORD_BUG,
    EC_WORD_SWARM,
    EC_WORD_RATHER,
    EC_POKEMON_NATIONAL(DKANAKO),
    EC_WORD_INVINCIBLE,
    EC_WORD_HEART,
    EC_WORD_MUFUFU,
    EC_WORD_MMM,
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseMe[] = {
    EC_MOVE2(CALM_MIND),
    EC_WORD_MAIL,
    EC_MOVE2(MEGA_DRAIN),
    EC_MOVE2(MEGA_KICK),
    EC_MOVE2(MEGA_PUNCH),
    EC_POKEMON_NATIONAL(NAMAZU),
    EC_MOVE2(MEGAHORN),
    EC_WORD_SEEK,
    EC_MOVE2(HIDDEN_POWER),
    EC_WORD_RARE,
    EC_POKEMON(CTOJIKO),
    EC_POKEMON_NATIONAL(KEDAMA),
    EC_MOVE2(METAL_CLAW),
    EC_POKEMON(SEIGA),
    EC_WORD_DESTROYED,
    EC_WORD_MESSAGE,
    EC_WORD_AWFULLY,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CMURASA),
    EC_POKEMON(CMURASA),
    EC_WORD_FIERY,
    EC_POKEMON_NATIONAL(HAKUREI),
    EC_MOVE2(ATTRACT),
    EC_WORD_CUTE_CHARM,
    EC_WORD_IMMUNITY,
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseMo[] = {
    EC_WORD_ALMOST,
    EC_WORD_BLAZE,
    EC_WORD_NEARLY,
    EC_WORD_MODE,
    EC_WORD_OLD,
    EC_WORD_THURSDAY,
    EC_POKEMON_NATIONAL(CKIRISAME),
    EC_WORD_LISTENING,
    EC_WORD_OWN,
    EC_WORD_MORE,
    EC_WORD_DIDN_T,
    EC_WORD_PICKUP,
    EC_MOVE2(MIMIC),
    EC_WORD_TOO_LATE,
    EC_WORD_PATTERN,
    EC_WORD_FLASH_FIRE,
    EC_WORD_RECEIVED,
    EC_POKEMON_NATIONAL(KOISHI),
    EC_WORD_DOESN_T,
    EC_POKEMON_NATIONAL(CSUNNYMILK),
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseYa[] = {
    EC_WORD_HOO_HAH,
    EC_WORD_PROMISE,
    EC_WORD_KIND,
    EC_POKEMON(HKOAKUMA),
    EC_WORD_HOLIDAY,
    EC_WORD_NEGATIVE,
    EC_MOVE2(FRUSTRATION),
    EC_WORD_DONE,
    EC_WORD_FINALLY,
    EC_WORD_ANY,
    EC_WORD_YAHOO,
    EC_POKEMON_NATIONAL(RIKAKO),
    EC_POKEMON_NATIONAL(CEIRIN),
    EC_MOVE2(LEECH_SEED),
    EC_POKEMON_NATIONAL(SATORI),
    EC_WORD_DANGER,
    EC_POKEMON_NATIONAL(CRIKAKO),
    EC_POKEMON(ADMYSTIA),
    EC_WORD_DEFEATED,
    EC_WORD_BEAT,
    EC_WORD_VITAL_SPIRIT,
    EC_POKEMON(ALILYWHITE),
    EC_WORD_YOUNG,
    EC_POKEMON_NATIONAL(MUGETSU),
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseYu[] = {
    EC_POKEMON(DPATCHOULI),
    EC_WORD_TAKE,
    EC_WORD_UNION,
    EC_MOVE2(METRONOME),
    EC_WORD_DREAM,
    EC_MOVE2(DREAM_EATER),
    EC_WORD_ALLOW,
    EC_WORD_FORGET,
    EC_POKEMON(ADRAN),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CPATCHOULI),
    EC_POKEMON(CPATCHOULI),
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseYo[] = {
    EC_WORD_WITHOUT,
    EC_WORD_YO,
    EC_MOVE2(ACID),
    EC_WORD_COME_OVER,
    EC_WORD_KINDERGARTEN,
    EC_WORD_CHLOROPHYLL,
    EC_WORD_AFTER,
    EC_POKEMON_NATIONAL(DMOMIJI),
    EC_WORD_OH_KAY,
    EC_WORD_BEFORE,
    EC_WORD_GREAT,
    EC_MOVE2(MEDITATE),
    EC_WORD_PURE_POWER,
    EC_WORD_WELL,
    EC_MOVE2(SNATCH),
    EC_WORD_OKAY,
    EC_WORD_PLANS,
    EC_WORD_NIGHTTIME,
    EC_WORD_WHILE,
    EC_POKEMON(RINNOSUKE),
    EC_WORD_EASY,
    EC_WORD_INSTEAD,
    EC_WORD_NIGHT,
    EC_POKEMON_NATIONAL(CELLEN),
    EC_WORD_COUNT_ON,
    EC_WORD_WEAK,
    EC_WORD_TOO_WEAK,
    EC_WORD_WEAKENED,
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseRa[] = {
    EC_POKEMON_NATIONAL(SHINGYOKUF),
    EC_WORD_WEEK,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CIRNO),
    EC_POKEMON(CIRNO),
    EC_WORD_RIVAL,
    EC_WORD_LIFE,
    EC_POKEMON(TTOYOHIME),
    EC_WORD_SIMPLE,
    EC_WORD_PUSHOVER,
    EC_POKEMON(TCIRNO),
    EC_POKEMON(ASHIKIEIKI),
    EC_WORD_SEEMS,
    EC_WORD_RADIO,
    EC_MOVE2(LUSTER_PURGE),
    EC_POKEMON_NATIONAL(ICHIRIN),
    EC_POKEMON_NATIONAL(NAZRIN),
    EC_POKEMON(MAMIZOU),
    EC_POKEMON(CKASEN),
    EC_POKEMON(AKISISTERS),
    EC_POKEMON_NATIONAL(TEWI),
    EC_WORD_LOVEY_DOVEY,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(AKEINE),
    EC_POKEMON(AKEINE),
    EC_WORD_LALALA,
    EC_POKEMON(TENMA),
    EC_WORD_FORGETS,
    EC_WORD_APPEARS,
    EC_WORD_RANK,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(SARA),
    EC_POKEMON(SARA),
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseRi[] = {
    EC_WORD_LEADER,
    EC_MOVE2(LEAF_BLADE),
    EC_POKEMON_NATIONAL(CREISEN),
    EC_POKEMON_NATIONAL(REISEN),
    EC_MOVE2(RECYCLE),
    EC_MOVE2(REFLECT),
    EC_MOVE2(REFRESH),
    EC_MOVE2(REVENGE),
    EC_WORD_RIBBON,
    EC_WORD_TRENDY,
    EC_MOVE2(DRAGON_RAGE),
    EC_MOVE2(DRAGON_BREATH),
    EC_MOVE2(DRAGON_DANCE),
    EC_WORD_VACATION,
    EC_POKEMON(ADYUKARI),
    EC_POKEMON_NATIONAL(HMYSTIA),
    EC_WORD_SHIELD_DUST,
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseRu[] = {
    EC_POKEMON_NATIONAL(CSHIKIEIKI),
    EC_WORD_ROOM,
    EC_WORD_RULE,
    EC_POKEMON_NATIONAL(SKOMACHI),
    EC_WORD_LOOK,
    EC_POKEMON(HLETTY),
    EC_WORD_RUBY,
    EC_WORD_THAN,
    EC_POKEMON(CSHINGYOKU),
    EC_POKEMON(SSHIZUHA),
    EC_WORD_HAPPILY,
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseRe[] = {
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CRAN),
    EC_POKEMON(CRAN),
    EC_MOVE2(ICE_PUNCH),
    EC_MOVE2(ICE_BEAM),
    EC_POKEMON(CFUTO),
    EC_POKEMON(FUTO),
    EC_POKEMON(TOJIKO),
    EC_POKEMON(CMAMIZOU),
    EC_WORD_UGLY,
    EC_POKEMON_NATIONAL(CKAZAMI),
    EC_POKEMON_NATIONAL(ELLEN),
    EC_WORD_FANTASTIC,
    EC_WORD_LEVEL,
    EC_MOVE2(FURY_CUTTER),
    EC_MOVE2(COMET_PUNCH),
    EC_WORD_RENTAL,
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseRo[] = {
    EC_MOVE2(SKULL_BASH),
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CSUIKA),
    EC_POKEMON(CSUIKA),
    EC_POKEMON(ADREISEN),
    EC_MOVE2(LOCK_ON),
    EC_MOVE2(ROCK_BLAST),
    EC_WORD_ROMANTIC,
    EC_WORD_QUESTION,
};

const u16 gEasyChatWordsByLetter_UnusedJapaneseWa[] = {
    EC_WORD_YAY,
    EC_WORD_WORLD,
    EC_WORD_AWW,
    EC_WORD_WIRELESS,
    EC_WORD_WOWEE,
    EC_POKEMON(TALICE),
    EC_WORD_UNDERSTAND,
    EC_WORD_UNDERSTANDS,
    EC_WORD_FAINT,
    EC_WORD_ANTICIPATION,
    EC_WORD_ONCE,
    EC_WORD_MOVE,
    EC_WORD_WE_VE,
    EC_WORD_FAINTED,
    EC_WORD_WOMAN,
    EC_WORD_SHE,
    EC_WORD_SHE_WAS,
    EC_WORD_TO_HER,
    EC_WORD_HERS,
    EC_WORD_SHE_IS,
    EC_WORD_SOME,
    EC_POKEMON_NATIONAL(RUUKOTO),
    EC_MOVE2(COTTON_SPORE),
    EC_WORD_GWAH,
    EC_POKEMON_NATIONAL(ORANGE),
    EC_WORD_WAHAHAHA,
    EC_WORD_ANYWHERE,
    EC_MOVE2(STRUGGLE),
    EC_WORD_BADLY,
    DOUBLE_SPECIES_NAME
    EC_POKEMON_NATIONAL(CYUUGI),
    EC_POKEMON(CYUUGI),
};

const struct EasyChatWordsByLetter gEasyChatWordsByLetterPointers[EC_NUM_ALPHABET_GROUPS + 18] = { // 18 unused JP groups
    {
        .words = gEasyChatWordsByLetter_Others,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_Others),
    },
    {
        .words = gEasyChatWordsByLetter_A,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_A),
    },
    {
        .words = gEasyChatWordsByLetter_B,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_B),
    },
    {
        .words = gEasyChatWordsByLetter_C,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_C),
    },
    {
        .words = gEasyChatWordsByLetter_D,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_D),
    },
    {
        .words = gEasyChatWordsByLetter_E,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_E),
    },
    {
        .words = gEasyChatWordsByLetter_F,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_F),
    },
    {
        .words = gEasyChatWordsByLetter_G,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_G),
    },
    {
        .words = gEasyChatWordsByLetter_H,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_H),
    },
    {
        .words = gEasyChatWordsByLetter_I,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_I),
    },
    {
        .words = gEasyChatWordsByLetter_J,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_J),
    },
    {
        .words = gEasyChatWordsByLetter_K,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_K),
    },
    {
        .words = gEasyChatWordsByLetter_L,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_L),
    },
    {
        .words = gEasyChatWordsByLetter_M,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_M),
    },
    {
        .words = gEasyChatWordsByLetter_N,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_N),
    },
    {
        .words = gEasyChatWordsByLetter_O,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_O),
    },
    {
        .words = gEasyChatWordsByLetter_P,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_P),
    },
    {
        .words = gEasyChatWordsByLetter_Q,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_Q),
    },
    {
        .words = gEasyChatWordsByLetter_R,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_R),
    },
    {
        .words = gEasyChatWordsByLetter_S,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_S),
    },
    {
        .words = gEasyChatWordsByLetter_T,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_T),
    },
    {
        .words = gEasyChatWordsByLetter_U,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_U),
    },
    {
        .words = gEasyChatWordsByLetter_V,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_V),
    },
    {
        .words = gEasyChatWordsByLetter_W,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_W),
    },
    {
        .words = gEasyChatWordsByLetter_X,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_X),
    },
    {
        .words = gEasyChatWordsByLetter_Y,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_Y),
    },
    {
        .words = gEasyChatWordsByLetter_Z,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_Z),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseHi,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseHi),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseFu,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseFu),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseHe,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseHe),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseHo,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseHo),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseMa,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseMa),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseMi,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseMi),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseMu,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseMu),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseMe,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseMe),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseMo,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseMo),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseYa,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseYa),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseYu,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseYu),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseYo,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseYo),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseRa,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseRa),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseRi,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseRi),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseRu,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseRu),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseRe,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseRe),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseRo,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseRo),
    },
    {
        .words = gEasyChatWordsByLetter_UnusedJapaneseWa,
        .numWords = ARRAY_COUNT(gEasyChatWordsByLetter_UnusedJapaneseWa),
    },
};
