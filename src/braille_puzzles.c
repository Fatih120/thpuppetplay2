...
static const u8 gCFutoPathCoords[][2] =
{
    {4,  21},
    {5,  21},
    {6,  21},
...
void DoBrailleTojikoEffect(void);
void DoBrailleFutoEffect(void);
...
{
    /*
        if (!FlagGet(FLAG_SYS_TOJIKO_PUZZLE_COMPLETED) && (gSaveBlock1.location.mapGroup == MAP_GROUP_DESERT_RUINS && gSaveBlock1.location.mapNum == MAP_ID_DESERT_RUINS))
    {
...
{
    if (!FlagGet(FLAG_SYS_FUTO_PUZZLE_COMPLETED) && (gSaveBlock1.location.mapGroup == MAP_GROUP_ANCIENT_TOMB && gSaveBlock1.location.mapNum == MAP_ID_ANCIENT_TOMB))
    {
...
bool8 ShouldDoBrailleTojikoEffect(void)
{
        ...
        && gSaveBlock1Ptr->location.mapNum == MAP_NUM(DESERT_RUINS))
    {
        ...
        {
            sBraillePuzzleCallbackFlag = TOJIKO_PUZZLE;
            return TRUE;
        }
...
bool8 ShouldDoBrailleFutoEffect(void)
{
    if (!FlagGet(FLAG_SYS_FUTO_PUZZLE_COMPLETED) && (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(ANCIENT_TOMB) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(ANCIENT_TOMB)))
    {
        ...
        {
            sBraillePuzzleCallbackFlag = FUTO_PUZZLE;
            return TRUE;
        }
    }
    return FALSE;
...
void Task_BrailleWait(u8 taskId)
{
    ...
    case 4:
        sub_8064E2C();
        ScriptContext1_SetupScript(S_OpenCFutoChamber);
        DestroyTask(taskId);
        break;
    }
}
...
bool8 ShouldDoBrailleCFutoPuzzle(void)
{
        ...
        && gSaveBlock1Ptr->location.mapNum == MAP_NUM(ISLAND_CAVE))
    {
        ...
        for (i = 0; i < 36; i++)
        {
            u8 xPos = gCFutoPathCoords[i][0];
            u8 yPos = gCFutoPathCoords[i][1];
