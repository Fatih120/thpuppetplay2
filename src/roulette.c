...
static void FlashSelectionOnWheel(u8 selectionId)
{
    ...
    default:
    {
        ...
        case COL_AMINORIKO:
            palOffset = PLTT_ID(gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_ORANGE_AMINORIKO]].oam.paletteNum);
            break;
        case COL_CSHINGYOKU:
            palOffset = PLTT_ID(gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_GREEN_CSHINGYOKU]].oam.paletteNum);
            break;
        case COL_ASUNNYMILK:
            palOffset = PLTT_ID(gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_PURPLE_ASUNNYMILK]].oam.paletteNum);
            break;
        case COL_AFLANDRE:
            palOffset = PLTT_ID(gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_ORANGE_AFLANDRE]].oam.paletteNum);
            break;
        }
        if (numSelected == 1)
        {
...
static void SetBallStuck(struct Sprite *sprite)
{
    ...
    {
        // If the player has the corresponding Pokémon in their party (HAS_DSUWAKO or HAS_TSHOU),
